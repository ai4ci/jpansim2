...
/**
 * Utility class providing mathematical conversions for probability, rates, and odds transformations.
 * This class implements conversions commonly used in epidemiological and statistical modelling.
 *
 ...
 * \]
 * <ul>
 *   <li><b>Exponential Distribution:</b> Probability from rate: \[ \prob = 1 - e^{-\rate} \]</li>
 *   <li><b>Logistic Transformation:</b> Logit function: \[ \text{logit}(\prob) = \ln\left(\frac{\prob}{1-\prob}\right) \]</li>
 *   <li><b>Odds Ratio Scaling:</b> Adjusted probability: \[ \text{logit}(\prob_{\text{adj}}) = \text{logit}(\prob) + \ln(OR) \]</li>
 * </ul>
 *
 * <p><b>Domain Constraints:</b>
 * <ul>
 ...
 * </ul>
 *
 * <p>All methods handle edge cases appropriately, returning appropriate limits for boundary inputs.
 *
 * @see Math#exp(double)
 * @see Math#log(double)
 */
public class Conversions {
	...
	/**
	 * Converts an average time period to the per-unit-time probability of transition
	 * under an exponential distribution model.
	 * \[
	 * \prob = 1 - e^{-1/\period}
	 * \]
	 *
	...
	/**
	 * Converts a probability to the equivalent average time period.
	 * Inverse of {@link #probabilityFromPeriod(double)}.
	 * \[
	 * \period = -\frac{1}{\ln(1-\prob)}
	 * \]
	 *
	...
	/**
	 * Converts a rate parameter to the per-unit-time probability of event occurrence
	 * under an exponential distribution assumption.
	 * \[
	 * \prob = 1 - e^{-\rate}
	 * \]
	 * Commonly used to convert transmission rates to daily probabilities.
	 *
	...
	/**
	 * Converts a probability to the equivalent rate parameter.
	 * Inverse of {@link #probabilityFromRate(double)}.
	 * \[
	 * \rate = -\ln(1-\prob)
	 * \]
	 *
	...
	/**
	 * Calculates the rate parameter given a specific quantile of the exponential distribution.
	 * Useful when specifying durations using percentiles rather than means.
	 * \[
	 * \rate = -\frac{\ln(1-q)}{\period}
	 * \]
	 *
	...
	/**
	 * Calculates the probability from a specified quantile and period.
	 * \[
	 * \prob = 1 - \exp\left(-\frac{\ln(1-q)}{\period}\right)
	 * \]
	 *
	...
	/**
	 * Calculates half-life from probability.
	 * Inverse of {@link #probabilityFromHalfLife(double)}.
	 * \[
	 * T_{1/2} = \frac{\ln(0.5)}{\ln(1-\prob)}
	 * \]
	 *
	...
	/**
	 * Scales a rate by an odds ratio using logistic transformation.
	 * Applies the odds ratio on the log-odds scale.
	 *
	...
	/**
	 * Calculates the odds ratio between two probabilities.
	 * \[
	 ...
	 * \[
	 * p_{\text{adj}} = p \cdot RR
	 * \]
	 * Note: Result may exceed 1.0 if RR > 1/p.
	 *
	 * @param p original probability
	 ...
	 * Inverse of {@link #probabilityFromOdds(double)}.
	 * \[
	 * \odds = \frac{p}{1-p}
	 * \]
	 *
	...
	/**
	 * Logistic transformation (logit function).
	...
	/**
	 * Inverse logistic transformation (expit function).
	...
	/**
	 * Multiplies two probabilities on the log-odds scale.
	 * Equivalent to independent probability multiplication.
	 * \[
	 * \text{logit}(p_1 \cdot p_2) = \text{logit}(p_1) + \text{logit}(p_2)
	 * \]
	 *
	...
	/**
	 * Applies exponential waning to log-odds over time.
	 * Models diminishing effect with hyperbolic decay.
	 * \[
	 * \text{logOdds}_{\text{waned}} = \text{logOdds} \cdot \frac{x}{x + t}
	 * \]
	 *
	...
	/**
	 * Calculates rate ratio between two probabilities.
	 * Simple ratio on probability scale.
	 * \[
	 * RR = \frac{p_1}{p_2}
	 * \]
	 *
