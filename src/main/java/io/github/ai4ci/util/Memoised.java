...
public class Memoised<X, Y> implements Function<X, Y> {
	...
	/**
	 * Creates a memoized version of the given function. The returned function
	 * will cache results for previously computed inputs, improving performance
	...
	@Override
	/**
	 * Applies the memoized function to the given input. If the result for the
	 * input is already cached, it returns the cached value. Otherwise, it
	 * computes the result using the original function, caches it, and returns
	 * it.
	 *
	 ...
	 */
	public Y apply(X t) {
		return this.cache.computeIfAbsent(t, this.fn);
	}
}