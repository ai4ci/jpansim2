 ...
 * Provides methods for calculating probabilities, odds, and confidence
 * intervals.
 *
 * @author Rob Challen
 */
public class Binomial extends MutablePair<Long, Long> {
	...
	public static class Confidence extends ImmutablePair<Double, Double> {
		 ...
		 *
		 * @return a formatted string showing the lower and upper bounds as
		 *         percentages
		 */
		@Override
		public String toString() {
			return String
					.format("[%.1f - %.1f]", this.left * 100, this.right * 100);
		}
	...
	public static Collector<Binomial, ?, Binomial> collect() {
		return Collector
				.of(Binomial::new, (p1, p2) -> p1.update(p2), Binomial::combine);
	}

	/**
	 * Returns a collector that converts boolean values increment numerator if
	 * true and denominator otherwise.
	 *
	 * @return a Collector for boolean values
	 */
	public static Collector<Boolean, ?, Binomial> collectBinary() {
		return Collector.of(
				Binomial::new, (p, b) -> p.update(b ? 1 : 0, 1), Binomial::combine
		);
	}

	/**
	 * Combines two Binomial instances by summing their numerators and
	 * denominators.
	 *
	...
	public static Binomial combine(Binomial left, Binomial right) {
		return Binomial.of(
				left.getLeft() + right.getLeft(), left.getRight() + right.getRight()
		);
	}
	...
	/**
	 * Checks if the binomial proportion is confidently greater than a specified
	 * value.
	 *
	 * @param p    the value to compare against
	 * @param conf the confidence level (e.g., 0.05 for 95% confidence)
	 * @return true if the lower bound of the confidence interval is greater than
	 *         p
	 */
	...
	/**
	 * Checks if the binomial proportion is confidently less than a specified
	 * value.
	 *
	 * @param p    the value to compare against
	 * @param conf the confidence level (e.g., 0.05 for 95% confidence)
	 ...
	 */
	public long getDenominator() { return this.getRight(); }
	 ...
	 */
	public long getNumerator() { return this.getLeft(); }
	 ...
	 *
	 * @return a formatted string with probability, confidence interval, and
	 *         counts
	 */
	@Override
	public String toString() {
		return String.format(
				"%.1f %s (%d/%d)", this.probability() * 100, this.wilson(0.05),
				this.left, this.right
		);
	}

	/**
	 * Updates the binomial counts by adding the counts from another Binomial
	 * instance.
	 *
	 * @param toAdd the Binomial instance to add
	 */
	public void update(Binomial toAdd) {
		this.update(toAdd.getLeft(), toAdd.getRight());
	}

	/**
	 * Updates the binomial counts by adding specified numerator and denominator
	 * values.
	 *
	...
	public Confidence wilson(double interval) {
		double z = NormalDistribution.of(0, 1)
				.inverseCumulativeProbability(1 - interval / 2);
		double p = this.probability();
		long n = this.right;
		double tmp = z / (2 * n) * Math.sqrt(4 * n * p * (1 - p) + z * z);
		double tmp2 = p + (z * z) / (2 * n);
		double tmp3 = 1 / (1 + (z * z) / n);
		return new Confidence(tmp3 * (tmp2 - tmp), tmp3 * (tmp2 + tmp));
	}
}