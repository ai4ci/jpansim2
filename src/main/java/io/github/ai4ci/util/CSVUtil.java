 ...
 * <p>
 * This class provides functionality to transform objects annotated with
 * {@link JsonProperty} into CSV rows with appropriate escaping for CSV special
 * characters. The transformation follows the standard CSV format specification
 * where fields may be quoted if they contain commas, quotes, or newlines.
 * <p>
 ...
 * </ul>
 *
 * @param <X> the type of objects to be serialised, must implement Writeable
 *            interface
 * @author Rob Challen
...
public class CSVUtil<X extends Writeable> {
	 ...
	 * <p>
	 * This static method converts individual values to CSV format and
	 * concatenates them with comma separators.
	 *
	 ...
	 * <p>
	 * This method applies the following rules to convert an object to a
	 * CSV-compatible string:
	 * <ul>
	 * <li>Null values are represented as "NA"</li>
	 * <li>Strings containing commas, quotes, or newlines are enclosed in double
	 * quotes</li>
	 ...
	 * with {@link JsonProperty} in the provided class. These methods are used to
	 * extract property values during CSV serialisation.
	 *
...
//		this.dt = ReflectionUtils.datatype(clazz);
	 ...
	 * <p>
	 * This method provides access to the property values as a stream, allowing
	 * for further stream processing operations. The stream is constructed from
	 * the array of values obtained by invoking annotated methods on the object
	 * instance.
	 *
	...
	public Stream<Object> fieldsStream(X tmp) {
		return Arrays.stream(this.valuesOf(tmp));
	}
...
//    }
	 ...
	 * <p>
	 * This method extracts property values from the object and converts them to
	 * CSV format.
	 *
	...
	public String row(X value) {
		return this.fieldsStream(value).map(o -> csvString(o))
				.collect(Collectors.joining(","));
	}
	 ...
	 * <p>
	 * This method invokes all methods annotated with {@link JsonProperty} on the
	 * specified object to obtain the corresponding property values. The values
	 * are returned in the same order as the methods array.
	 *
	 *
...
//					.toArray();
//		}
		Object[] out = new Object[this.methods.length];
		for (int i = 0; i < this.methods.length; i++) {
			try {
				out[i] = this.methods[i].invoke(single);
