 ...
 *
 * <p>
 * Writes can be performed concurrently from multiple threads using
 * {@link #put(Object)}. Once all writes are complete, call {@link #finish()} to
 * seal the array and obtain an immutable, fixed-size snapshot for reading.
 * </p>
 *
 * <p>
 * This structure assumes two distinct phases:
 * <ol>
 * <li><strong>Writing phase:</strong> Multiple threads may call
 * {@link #put(Object)} or {@link #update(int,Object)} concurrently.</li>
 * <li><strong>Reading phase:</strong> After calling {@link #finish()}, only
 * read operations are allowed via {@link #get(int)}, or streaming methods like
 * {@link #stream()} and {@link #parallelStream()}.</li>
 * </ol>
 *
 * <p>
 * The array is dynamically resized if needed during writes. Resizing is rare
 * and synchronized internally, but has minimal impact on throughput when the
 * final size is preallocated.
 * </p>
 *
 * <p>
 * {@link #finish()} blocks until all pending writes have completed before
...
public class ThreadSafeArray<X> implements Serializable {
	...
	/**
	 * Thrown when attempting to write to a {@link ThreadSafeArray} after it has
	 * been finalized with a call to {@link #finish()}.
	 */
	public static class ReadOnlyException extends RuntimeException {}

	/**
	 * Thrown when attempting to write to a {@link ThreadSafeArray} while it is
	 * being resized or finalized. This exception is not currently used, but
	 * reserved for future internal use.
	 */
	public static class WriteOnlyException extends RuntimeException {}

	/**
	 * Creates an empty, thread-safe array of the given type and zero initial
	 * capacity.
	 *
	...
	public static <Y> ThreadSafeArray<Y> empty(Class<Y> type) {
		return new ThreadSafeArray<>(type, 0);
	}
	...
	// the array is busy doing a bulk operation. Appends or updates will be
	// onSpinWaited.
	volatile AtomicBoolean updating = new AtomicBoolean();

	/**
	 * Constructs a new thread-safe array with the given element type and initial
	 * capacity.
	 *
	 * <p>
	 * If the final size is known ahead of time, it is recommended to preallocate
	 * the array to minimize resizing overhead during concurrent writes.
	 * </p>
	 *
	 * @param type the runtime class of the array elements
	...
	private void ensureCapacityOrWait(int p) {
		...
		while (p >= this.data.length) {
			...
			if (this.updating.compareAndSet(false, true)) {
				...
				while (p >= this.data.length && this.updating.get()) {
					Thread.onSpinWait();
				}
			}
			// capacity is large enough or updates have finished (and capacity is
			// still too small).
		}
	 ...
	 *
	 * <p>
	 * This method blocks until all concurrent writes in progress have completed.
	 * </p>
	 *
	 * <p>
	 * After calling this method, any further calls to {@link #put(Object)} will
	 * throw a {@link ReadOnlyException}.
	 * </p>
	 *
	 * @return a fixed-size copy of the array containing all elements
	 */
	public X[] finish() {
		while (!this.updating.compareAndSet(false, true)) {
			Thread.onSpinWait();
		}
		// This thread is updating. No more additions should be made.
		// waits for any lagging writer threads to catch up
		while (this.written.get() < this.pointer.get()) {
			Thread.onSpinWait();
		}
	 ...
	 *
	 * <p>
	 * If called before {@link #finish()}, this method throws an
	 * {@link ArrayIndexOutOfBoundsException} if the element does not yet exist.
	 * </p>
	 *
	 * <p>
	 * Callers should ensure that the index is within bounds as reported by
	 * current write progress (e.g., via {@link #size()}).
	 * </p>
	 *
	...
	public X get(int p) {
		...
		if (p >= this.pointer.get()) throw new ArrayIndexOutOfBoundsException();
		return this.data[p];
	}
	 ...
	 *
	 * <p>
	 * Like {@link #stream()}, this reflects the ideal state of the array at the
	 * time of the call. It may have to wait for items to become fully available.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * This method is safe to call from multiple threads concurrently. If the
	 * internal array needs resizing, only one thread will perform the resize
	 * operation while others wait.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * This value may be less than the actual array capacity due to dynamic
	 * resizing.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * The stream reflects the ideal state of the array at the time of the call.
	 * It may have to wait for all items to become available.
	 * </p>
	 *
	 ...
	 * exists, and the initial loading phase is complete but the array has not
	 * been finalised.
	 *
	 * <p>
	 * This method is thread-safe and supports concurrent writes to different
	 * indices.
	 * </p>
	 *
	 * @param index the index at which to update the element
	 * @param value the element to update
	 * @throws ReadOnlyException         if called after the array has been
	 *                                   finalized
	...
	public void update(int index, X value) {
		...
		if (index >= this.pointer.get()) throw new IndexOutOfBoundsException();
