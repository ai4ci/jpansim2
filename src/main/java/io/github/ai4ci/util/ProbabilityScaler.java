...
/**
 * Utility class for scaling probabilities using power transformations. Provides
 * methods to find the appropriate scaling factor k such that the scaled
 * probabilities sum to a desired target, as well as direct scaling of
 * probabilities using a given k.
 *
 * The main method findScalingFactor uses Newton-Raphson iteration with a
 * bisection fallback to robustly solve for the scaling factor.
 *
 * Example usage:
 *
 * <pre>
...
public class ProbabilityScaler {
	...
	private static double bisectionSolve(
			double[] p, double targetSum, double tol, int maxIter
	) {
		...
		for (int iter = 0; iter < maxIter; iter++) {
			...
			if (Math.abs(error) < tol) return alphaMid;
			if (error > 0) {
	...
	private static double computeSum(double[] p, double alpha) {
		double sum = 0.0;
		for (double pi : p) {
			if (pi > 1e-15) { sum += Math.pow(pi, alpha); }
	 ...
	 * @param targetSum     Desired sum of scaled probabilities (must be in
	 *                      feasible range)
	 * @return Scaling factor k > 0 such that sum(p_i^(1/k)) ≈ targetSum
	 * @throws IllegalArgumentException if inputs are invalid or targetSum is
	 *                                  infeasible
	 *
	 */
	public static double findScalingFactor(
			double[] probabilities, double targetSum
	) {
		return findScalingFactor(probabilities, targetSum, 1e-8, 50);
	}

	/**
	 * Scales probabilities using power transformation: q_i = p_i^(1/k) Finds k
	 * such that sum(q_i) = targetSum using Newton-Raphson with bisection
	 * fallback.
	 *
	 * @param probabilities Array of probabilities in [0, 1]
	 * @param targetSum     Desired sum of scaled probabilities (must be in
	 ...
	 */
	public static double findScalingFactor(
			double[] probabilities, double targetSum, double tolerance,
			int maxIterations
	) {
		...
		if (Math.abs(targetSum - sumOriginal) < tolerance) return 1.0;
		...
		if (targetSum <= sMin + tolerance) throw new IllegalArgumentException(
				String.format(
						"Target sum %.6f is below feasible minimum %.6f", targetSum,
						sMin
				)
		);
		if (targetSum >= sMax - tolerance) throw new IllegalArgumentException(
				String.format(
						"Target sum %.6f is above feasible maximum %.6f", targetSum,
						sMax
				)
		);

		// Solve for alpha = 1/k using Newton-Raphson
		double alpha = solveForAlpha(
				probabilities, targetSum, tolerance, maxIterations
		);
	...
	public static double[] scaleProbabilities(double[] probabilities, double k) {
		if (k <= 0) throw new IllegalArgumentException(
				"Scaling factor k must be positive"
		);
		double alpha = 1.0 / k;
	 ...
	 *                      feasible range)
	 */
	public static double[] scaleToTargetSum(
			double[] probabilities, double targetSum
	) {
	...
	private static double solveForAlpha(
			double[] p, double targetSum, double tol, int maxIter
	) {
		...
		double pGeom = (nonZeroCount > 0) ? Math.exp(logSum / nonZeroCount)
				: 1e-15;
		double alpha = Math.log(targetSum / p.length) / Math.log(pGeom);
		alpha = Math.max(1e-6, Math.min(100.0, alpha)); // Clamp to reasonable
																		// range
		...
		for (int iter = 0; iter < maxIter; iter++) {
			...
			for (double pi : p) {
				if (pi < 1e-15) {
					continue; // 0^alpha = 0, derivative = 0
				}
			...
			if (Math.abs(error) < tol) return alpha;
			...
			if (alphaNew <= 0) {
				break; // Invalid alpha → switch to bisection
			}
			alpha = alphaNew;
		}
	...
	private static void validateInputs(double[] p, double targetSum) {
		if (p == null || p.length == 0) throw new IllegalArgumentException(
				"Probabilities array cannot be null or empty"
		);
		if (targetSum <= 0 || targetSum >= p.length)
			throw new IllegalArgumentException(
					String.format(
							"Target sum must be in (0, %d), got %.6f", p.length,
							targetSum
					)
			);
		for (int i = 0; i < p.length; i++) {
			if (p[i] < 0 || p[i] > 1) throw new IllegalArgumentException(
					String.format(
							"Probability at index %d is %.6f; must be in [0, 1]", i,
							p[i]
					)
			);
		}
