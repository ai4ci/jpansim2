 ...
 *
 * <p>
 * Main purpose: provide a small value type that accepts raw x/y arrays, applies
 * domain and range link functions, filters points to a bounded support and
 * produces cleaned coordinate arrays suitable for monotone spline fitting.
 *
 * <p>
 * Key features:
 * <ul>
 ...
 * {@link io.github.ai4ci.functions.EmpiricalDistribution} when preparing data
 * for spline fitting, and by
 * {@link io.github.ai4ci.functions.SplineInterpolator} for constructing
...
public interface Coordinates {
	...
	public static enum DuplicateResolution {
		/**
		 * When multiple original points map to the same transformed x value,
		 * compute the mean of their y values.
		 */
		MEAN,
		/**
		 * When multiple original points map to the same transformed x value, take
		 * the maximum of their y values.
		 */
		MAX,
		/**
		 * When multiple original points map to the same transformed x value, take
		 * the minimum of their y values.
		 */
		MIN
	}

	/**
	 * Returns the transformed x-coordinates after applying the x link and
	 * aggregating duplicates according to {@link #resolveDuplicates()}.
	 *
	 * @return an array of transformed x values suitable for spline fitting
	 */
	@Value.Derived @JsonIgnore
	public default double[] getHx() {
		return this.getMapping().keySet().stream().mapToDouble(d -> d).toArray();
	}

	/**
	 * Returns the transformed y values corresponding to {@link #getHx()}. If
	 * multiple original points map to the same transformed x the values are
	 * combined according to {@link #resolveDuplicates()}.
	 *
	 * @return an array of transformed y values aligned with {@link #getHx()}
	 */
	@Value.Derived @JsonIgnore
	public default double[] getHy() {
		return this.getMapping().values().stream().mapToDouble(list -> {
			if (this.resolveDuplicates().equals(DuplicateResolution.MEAN)) {
				return list.stream().mapToDouble(d -> d).average().getAsDouble();
			}
			if (this.resolveDuplicates().equals(DuplicateResolution.MAX)) {
				return list.stream().mapToDouble(d -> d).max().getAsDouble();
			} else if (this.resolveDuplicates().equals(DuplicateResolution.MIN)) {
	...
	private SortedMap<Double, List<Double>> getMapping() {
		...
		for (var i = 0; i < this.getX().length; i++) {
			var x = this.getX()[i];
			var y = this.getY()[i];
			if (x <= this.getXMax() && x >= this.getXMin() && y <= this.getYMax()
					&& y >= this.getYMin()) {
				var tx = this.getXLink().fn(x);
				var ty = this.getYLink().fn(y);
				out.putIfAbsent(tx, new ArrayList<>());
				out.get(tx).add(ty);
			}
		}
		if (this.isIncreasing()) {
			out.putIfAbsent(
					this.getXLink().fn(this.getXMin()),
					List.of(this.getYLink().fn(this.getYMin()))
			);
			out.putIfAbsent(
					this.getXLink().fn(this.getXMax()),
					List.of(this.getYLink().fn(this.getYMax()))
			);
		}
		return out;
	}
	 ...
	 */
	@Value.Default
	default double getXMax() { return this.getXLink().getMaxSupport(); }
	 ...
	 */
	@Value.Default
	default double getXMin() { return this.getXLink().getMinSupport(); }

	/**
	 * The raw y coordinate array corresponding to {@link #getX()}.
	 *
	 * @return an array of y values where {@code getY()[i]} pairs with
	 *         {@code getX()[i]}
	 */
	public double[] getY();
	 ...
	 */
	@Value.Default
	default double getYMax() { return this.getYLink().getMaxSupport(); }
...
//	}
	 ...
	 */
	@Value.Default
	default double getYMin() { return this.getYLink().getMinSupport(); }

	/**
	 * Whether the x values are strictly increasing. If true additional boundary
	 * points based on {@link #getXMin()}/{@link #getXMax()} and
	 * {@link #getYMin()}/{@link #getYMax()} are inserted to guarantee full
	 * domain coverage for monotone interpolators.
	 *
	...
	/**
	 * Strategy used when multiple transformed x values collide after applying
	 * {@link #getXLink()} (duplicate transformed keys).
	 *
	 * @return the duplicate resolution policy
	 */
	@Value.Default
	default DuplicateResolution resolveDuplicates() {
		return DuplicateResolution.MEAN;
	}
