...
public interface EmpiricalFunction extends Serializable, SimpleFunction {
	...
	default double differential(double x) {
		return this.getLink().derivInvFn(this.getInterpolator().interpolate(x))
				* this.getInterpolator().differential(x);
	...
	default Interpolator getInterpolator() {
		...
		return this.getInterpolationType().create(tmp.getHx(), tmp.getHy());
