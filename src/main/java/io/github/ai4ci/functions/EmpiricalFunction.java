 ...
 *
 * <p>This interface describes a simple empirical function defined by a set of
 ...
 * such as the age contact odds function in {@link io.github.ai4ci.config.setup.AgeStratifiedDemography}.
 *
 * <p>Downstream uses include any place where an empirical function is required,
...
public interface EmpiricalFunction extends Serializable, SimpleFunction {
	...
	/**
	 * The link function used to transform the Y coordinates before interpolation.
	 * @return the link function used to transform the Y coordinates before interpolation
	 */
	@Value.Default default LinkFunction getLink() {return LinkFunction.NONE;}
	
	/**
	 * The interpolation method used to interpolate between the (x,y) coordinates.
	 * @return the interpolation method used to interpolate between the (x,y) coordinates
	 */
	@Value.Default default Interpolators getInterpolationType() {return Interpolators.CUBIC_SPLINE;}
	
	/**
	 * The minimum X value of the empirical function, derived from the coordinates.
	...
	/**
	 * The maximum X value of the empirical function, derived from the coordinates.
	 ...
	 * <p>
	 * This method constructs an interpolator based on the (x,y) coordinates and the
	 * specified interpolation type. It applies the link function to the Y
	 * coordinates before interpolation. The resulting interpolator can be used to
	 * compute interpolated values for any X within the range of the coordinates.
	 * 
	 * @return the interpolator created from the coordinates and interpolation type
	 */
	@JsonIgnore
	@Value.Derived 
	default Interpolator getInterpolator() {
		...
		return getInterpolationType().create(
				tmp.getHx(), tmp.getHy()
		);
	 ...
	 * interpolating the Y values at the coordinates and then applying the inverse
	 * link function to the interpolated value. The result is the value of the
	 * empirical function at the specified X.
	 * 
	 ...
	 * <p>
	 * This method computes the differential of the empirical function at a given X
	 * by applying the chain rule to the interpolated value and its derivative. It
	 * uses the derivative of the link function and the derivative of the
	 * interpolator to compute the overall differential at the specified X.
	 * 
	 * @param x the X value at which to compute the differential of the empirical function
