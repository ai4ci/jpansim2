 ...
 *
 * <p>Main purpose: provide a small set of commonly used transforms (identity,
 ...
 * and spline/ interpolation helpers to map raw values into a transformed
 * working scale and back again.
 *
 * @author Rob Challen
 */
public enum LinkFunction {
    ...
    private LinkFunction( DoubleUnaryOperator fn, DoubleUnaryOperator invFn, DoubleUnaryOperator derivFn,DoubleUnaryOperator derivInvFn, double lim) {
        this.fn = fn;
        this.invFn = invFn;
     ...
     * clipped to a pre-defined numeric bound to avoid infinities from
     * downstream spline operations.
     *
    ...
    public double fn(double x) {
        ...
        return Interpolator.squish(-lim, hx, lim);
    }
    /**
    ...
    public double invFn(double y) {
        y = Interpolator.squish(-lim, y, lim);
        return invFn.applyAsDouble(y);
    }

    /**
     * Return the derivative of the forward link evaluated at the supplied
     * domain value.
     *
    ...
    /**
     * Returns true if the supplied domain value lies strictly within the
     * numeric support limits for this link.
     *
     * @param x the value to test
     * @return {@code true} if the value is within support, otherwise {@code false}
     */
