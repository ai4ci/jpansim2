...
public interface OutbreakHistory extends OutbreakTemporalState {
	 ...
	 *
	 * @return An optional containing the previous history entry, or empty if this
	 *         is the first entry (time=0).
	 */
	 ...
	 *         observed in the history. This is used to determine how far forward to
	 *         look when calculating test positive/negative counts by sample date.
	 */
	@Value.Lazy
	 ...
	 * results and will tell us what is known about tests taken today on day X. Any
	 * positive value is counted as a test positive person on this day.
	 *
	 * @return the number of people with any positive test result reported for tests
	 *         taken on this day, as observed at the given time.
	 */
	 ...
	 * An observed value:<br>
	 * <br>
	 * The current state of knowledge about test positive results from samples taken
	 * on this day (potentially in the past). This is output as part of the delay
	 * distribution of tests, as it gives a positive test count for a given day that
	 * increases in the future. (N.B. This looks over complicated and I question its
	 * utility, it is also misleading as it is a count of people with any positive
	 * test)
	 *
	 * @return the number of people with any positive test result reported for tests
	 *         taken on this day, as observed at the current simulation time.
	 */
	 ...
	 * looking forward through time until the result is available. The result is a
	 * forward distribution of positive test counts indexed by delay. So first is
	 * the tests with positive results available today, second is those with results
	 * tomorrow etc.
	 *
	...
	default List<Integer> getTestPositivesBySampleDate() {
		return IntStream.range(0, getMaxDelay()).mapToObj(delay -> {
			return ModelNav.peopleHistory(this).mapToInt(p ->
			// If any of a persons results are positive today
			p.getResultsBySampleDate(this.getTime() + delay).anyMatch(t -> t.getFinalObservedResult()) ? 1 : 0).sum();
		}).collect(Collectors.toList());
	}

	/**
	 * The total test negatives reported from this day, as reported on another day
	 * (typically in the future). This handles delay distribution of test results
	 * and will tell us what is known about tests taken today on day x. All tests
	 * taken on this day must be negative for the person to count as a test
	 * negative.
	 *
	 * An observed value:<br>
	 *
	...
	/**
	 *
	 * The current state of knowledge about people with test negative results from
	 * this day (which is potentially in the past) as assessed at current simulation
	 * time. <br>
	 * An observed value:
	 *
	 * @return the number of people with all negative test results reported for
	 *         tests taken on this day, as observed at the current simulation time.
	 */
	...
	default List<Integer> getTestNegativesBySampleDate() {
		return IntStream.range(0, getMaxDelay()).mapToObj(delay -> {
			return ModelNav.peopleHistory(this).mapToInt(p -> {
				if (p.getTodaysTests().isEmpty()) {
					return 0;
				}
				// If any of a persons results are positive today
				return p.getResultsBySampleDate(this.getTime() + delay).allMatch(t -> !t.getFinalObservedResult()) ? 1
						: 0;
			}).sum();
		}).collect(Collectors.toList());
	}
