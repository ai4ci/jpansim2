...
public interface Outbreak extends Entity, HistoricalStateProvider<OutbreakHistory>, Cloneable {
	 ...
	 *
	 * <p>
	 * Format: {@code urnBase:setupName:setupReplica:execName:execReplica}. Used for
	 * logging, output filenames, and random seed derivation.
	 *
	 ...
	 *
	 * <p>
	 * Size is fixed at setup time and equals {@link #getPopulationSize()}. Indexed
	 * by agent ID (0-based).
	 * 
	 ...
	 *
	 * <p>
	 * Immutable after setup. Includes demographic model (e.g., age-stratified) and
	 * network generator (e.g., Erdős–Rényi).
	 * 
	 ...
	 *
	 * <p>
	 * Immutable after initialization. Includes in-host model, R₀, testing strategy,
	 * and policy triggers.
	 * 
	 ...
	 * {@code switchState()}). Cleared after each timestep. Used to stage the next
	 * outbreak state before finalizing into {@link #getCurrentState()} and
	 ...
	 * (e.g., test counts, incidence) before finalizing into {@link #getHistory()}.
	 * 
	 ...
	 *
	 * <p>
	 * Most recent entry (index 0) corresponds to {@link #getCurrentState()}. Length
	 * is capped to ~2× infectious period to limit memory use.
	 * 
	 * @return list of outbreak history entries, ordered from most recent to oldest
	 */
	@Override
	 ...
	 *         change during the simulation, but is thread-safe to allow concurrent
	 *         reads during contact processing
	 */
	...
	default double getAverageNetworkDegree() {
		return getSocialNetwork().size() * 2 / (double) getPopulationSize();
	}
	...
	/**
	 * underride equality to compare outbreak instances by their hash codes, which
	 * are random and unique per instance
	 *
	...
	/**
	 * underride toString to return the outbreak's URN for easier identification in
	 * logs and outputs
	 *
	...
	/**
	 * Returns the most recent historical state (same as current state during update
	 * cycle).
	 *
	 * @return an Optional containing the most recent OutbreakHistory, or empty if
	 *         history is empty
	 */
	 ...
	 * @return the agent's history record for that day, or {@code Optional.empty()}
	 *         if not found
	 */
