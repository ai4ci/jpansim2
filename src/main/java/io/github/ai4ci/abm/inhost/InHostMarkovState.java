...
/**
 * A discrete-time Markov chain model of in-host disease and symptom progression.
 *
 * <p>This model tracks two independent state machines:
 * <ul>
 *   <li><b>Disease state</b>: SUSCEPTIBLE → EXPOSED → INFECTIOUS → IMMUNE,</li>
 *   <li><b>Symptom state</b>: ASYMPTOMATIC → SYMPTOMATIC → HOSPITALISED → DEAD.</li>
 * </ul>
 *
 * <p>Transitions are governed by daily probabilities defined in {@link InHostMarkovStateMachine}.
 * The model assumes binary immunity: once immune, individuals are fully protected unless waning occurs.
 *
 * <p>The viral load, severity, and immune activity are derived from the current state, enabling
 * integration with transmission and calibration modules. This approach sacrifices biological detail
 * for computational efficiency and direct mapping to observable epidemiological rates (e.g., IFR, HFR).
 *
...
public interface InHostMarkovState extends InHostModelState<MarkovStateModel> {
	 ...
	 * <p>States evolve as:
	 * \[
	 * \text{SUSCEPTIBLE} \xrightarrow{p_{\text{inf}}} \text{EXPOSED} \xrightarrow{p_{\text{inf→inf}}} \text{INFECTIOUS} \xrightarrow{p_{\text{rec}}} \text{IMMUNE} \xrightarrow{p_{\text{waning}}} \text{SUSCEPTIBLE}
	 * \]
	 ...
	 *
	 * <p>These represent observable health outcomes:
	 * \[
	 * \text{ASYMPTOMATIC} \xrightarrow{p_s} \text{SYMPTOMATIC} \xrightarrow{p_h,p_d} \text{HOSPITALISED} \xrightarrow{p_{hd}} \text{DEAD}
	 * \]
	 * with possible regression to less severe states (e.g., recovery). The symptom state is conditional
	...
	/**
	 * Defines the transition probabilities for the Markov chain model of disease and symptom progression.
	 *
	 * <p>This component encapsulates all time-homogeneous transition rates between states.
	 * It enables calibration from epidemiological data (e.g., duration of infectiousness, case fatality rate).
	 *
	 * <p>The probability of becoming symptomatic is computed as:
	 * \[
	 * p_s = 1 - (1 - r_c)^{1/D}
	 * \]
	 * where \( r_c \) is the infection-case rate and \( D \) is the average duration of infection,
	 * ensuring that over the full infectious period, the cumulative probability of symptoms is \( r_c \).
	 *
	...
	public static interface InHostMarkovStateMachine extends Serializable {
		...
		/**
		 * Returns the daily probability of transitioning from asymptomatic to symptomatic,
		 * calibrated to achieve the target infection-case rate over the infectious period.
		 *
		 * <p>If \( r_c \) is the desired case rate and \( D \) is the average duration of infectiousness,
		 * then:
		 * \[
		 * p_s = 1 - (1 - r_c)^{1/D}
		 * \]
		 * This ensures that the probability of ever becoming symptomatic during infection is \( r_c \),
		 * under the assumption of independent daily risk.
		 *
		 * <p>This formulation assumes each day presents an independent chance of symptom onset,
		 * and the overall process resembles a <i>geometric trial</i> or a sum of Bernoulli variables
		 * with equal probability — a special case of the Poisson binomial distribution.
		 *
		 ...
		 * </ul>
		 *
		 * <p>The use of independent Bernoulli draws aligns with the discrete-time survival framework.
		 * When aggregating over a population, the number of individuals transitioning follows a
		 * <i>Poisson binomial distribution</i> with:
		 * \[
		 * \mathbb{E}[N_{\text{transitions}}] = \sum_i p_i, \quad \mathrm{Var}(N_{\text{transitions}}) = \sum_i p_i(1 - p_i)
		 * \]
		...
		/**
		 * Updates the symptom state using multinomial transitions conditioned on disease state.
		 *
		 * <p>For example, from SYMPTOMATIC, the individual may:
		 * <ul>
		 ...
		 * </ul>
		 *
		 * <p>The multinomial draw ensures only one transition occurs per day. The probabilities
		 * are calibrated to reproduce population-level outcomes (e.g., IFR) over time.
		 *
	 ...
	 * <p>Defined as:
	 * \[
	 * V_{\text{norm}} = \begin{cases}
	 *   1.5 &amp; \text{if } \text{state} = \text{INFECTIOUS} \\
	 *   0.5 &amp; \text{if } \text{state} = \text{EXPOSED} \\
	 *   0.0 &amp; \text{otherwise}
	 * \end{cases}
	 * \]
	 * This coarse proxy enables transmission modeling without detailed viral kinetics.
	 *
	...
	/**
	 * Samples a continuous severity score from a uniform distribution based on symptom state.
	 *
	 * <p>The score is calibrated so that:
	 * <ul>
	 ...
	 * </ul>
	 * where \( r_c, r_h, r_f \) are the infection-case, infection-hospitalization, and infection-fatality ratios.
	 *
	 * <p>For example:
	 * \[
	 * S \sim \text{Uniform}(1 - r_h, 1 - r_f) \quad \text{if } \text{HOSPITALISED}
	 * \]
	 * This ensures the population-level event rates match input parameters.
	 *
	 ...
	 * \[
	 * I(t) = \begin{cases}
	 *   1.0 &amp; \text{if } \text{disease state} = \text{IMMUNE} \\
	 *   0.0 &amp; \text{otherwise}
	 * \end{cases}
	 * \]
	 * This reflects full protection during the immune state, with no partial immunity.
	 *
	 ...
	 *   <li>Otherwise: apply Markov transitions from current disease and symptom states.</li>
	 * </ol>
	 *
	 * <p>Disease progression uses Bernoulli and multinomial sampling:
	 * \[
	 * \mathbb{P}(X_{t+1} = x') = \sum_{x} \mathbb{P}(X_t = x) \cdot p(x \to x')
	 * \]
	 * where transition probabilities are constant per day.
	 *
	 * <p>The use of {@link Sampler#bern(Double, Object)} and {@link Sampler#multinom(Pair...)} ensures
	 * independent stochastic realizations. The sum of such independent transitions aligns with
	 * the framework of the <i>Poisson binomial distribution</i> when aggregating over populations,
	 * where each individual has a potentially different path through the state space.
	 *
	 * @param sampler the random sampler for stochastic transitions
	 * @param virionExposure non-zero if the individual is exposed to infectious virions
	 * @param immunisationDose non-zero if the individual receives an immunizing dose (e.g., vaccine)
