...
import static io.github.ai4ci.flow.mechanics.StateUtils.doLFT;
...
import static io.github.ai4ci.flow.mechanics.StateUtils.isLFTTestingAllowed;
import static io.github.ai4ci.flow.mechanics.StateUtils.isPCRTestingAllowed;
import static io.github.ai4ci.flow.mechanics.StateUtils.isPositiveTestToday;
...
public enum SmartAgentLFTTesting implements BehaviourModel, DefaultNoTesting {
	 ...
	 * test. If they have a positive LFT, they will then get a PCR test to
	 * confirm, and isolate while awaiting the result.
	 */
	REACTIVE_LFT {
		...
		public void updateHistory(
			...
			if (isSymptomaticAndCompliant(person, 2)
				...
				if (isLFTTestingAllowed(person)) {
					doLFT(builder, person, context);
				}
			}
	...
	/**
	 * Patient will test with PCR if they have symptoms and a positive LFT, then
	 * wait for the result.
	 */
	REFLEX_PCR {
	...
	/**
	 * Patient will isolate if they have a positive PCR, but will wait for the
	 * result.
	 */
	AWAIT_PCR {
		@Override
