...
/**
 * Defines parameters for medical diagnostic tests used in epidemiological
 * simulations.
 *
 * <p>
 * Models key characteristics of diagnostic tests including:
 * <ul>
 ...
 * </ul>
 *
 * The testing assumes a viral load signal that is normalised but unbounded. The
 * limit of detection is used to determine the point at which the test can
 * reliably detect infection. A true viral load at the limit of detection will
 * produce a true positive rate equal to the test sensitivity, and a true viral
 * load of zero will produce a false positive rate equal to (1 - specificity).
 * Very high viral loads will produce a true positive rate approaching 1, but
 * the false positive rate will remain at (1 - specificity) regardless of viral
 * load.
 *
 *
 * <p>
 * Used to simulate test accuracy and results processing in disease transmission
 * models. Implements statistical methods for calculating likelihood ratios and
 * applying test noise.
 *
 * @author Rob Challen
 */
...
public interface TestParameters extends Serializable {
	...
	/**
	 * Applies diagnostic test noise to a normalized signal based on test
	 * characteristics.
	 *
	 * <p>
	 * Implements a uniform noise model where:
	 * <ul>
	 * <li>Output for zero signal should be &lt; 1 in (specificity) cases</li>
	 * <li>Output for signal at limit of detection should be 1 in (1-sensitivity)
	 * cases</li>
	 * </ul>
	 *
	 * <p>
	 * The noise follows a linear cumulative distribution between: - Point 0:
	 * (1-sensitivity) - Point 1: (1-specificity)
	 *
	 ...
	 */
	static double applyNoise(
			double normalisedSignal, double sensitivity, double specificity,
			double limitOfDetection, Sampler rng
	) {
		...
		return Math.max(
				0,
				normalisedSignal
						+ limitOfDetection * (rng.uniform() - (1 - sensitivity))
	...
	default double applyNoise(double normViralLoad, Sampler rng) {
		return applyNoise(
				normViralLoad, this.getSensitivity(), this.getSpecificity(),
				this.getLimitOfDetection(), rng
		);
	}
	 ...
	 *
	 * @return Value between 0 and 1 representing probability of positive result
	 *         when infected
	 */
	 ...
	 *
	 * @return Value between 0 and 1 representing probability of negative result
	 *         when not infected
	 */
	...
	/**
	 * Calculates the negative likelihood ratio (LR-). LR- = (1 - Sensitivity) /
	 * Specificity
	 *
	 * @return Ratio indicating how much to decrease probability of disease with
	 *         negative result
...
//	}
	...
	/**
	 * Calculates the positive likelihood ratio (LR+). LR+ = Sensitivity / (1 -
	 * Specificity)
	 *
	 * @return Ratio indicating how much to increase probability of disease with
	 *         positive result
