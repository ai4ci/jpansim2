...
/**
 * Enumeration of the different exporters available in the system, each associated with an export selector.
 *
 * <p>This enum provides a convenient way to reference and manage the various exporters used for outputting
 * different types of data during the simulation. Each exporter is linked to a specific CSV format and
 * export stage, as defined by their respective selectors.
 *
 * @author Rob Challen
 */
public enum Exporters {
	...
	/** Exporter for historical test data, using the ImmutableOutbreakHistoryCSV format. */
	...
	/** Exporter for infectivity profile data, using the ImmutableInfectivityProfileCSV format. */
	INFECTIVITY_PROFILE (ExportSelector.of(ImmutableInfectivityProfileCSV.class)),
	/** Exporter for person demographic data, using the ImmutablePersonDemographicsCSV format. */
	DEMOGRAPHICS (ExportSelector.of(ImmutablePersonDemographicsCSV.class)),
	/** Exporter for debug parameters, using the ImmutableDebugParametersCSV format. */
	DEBUG_PARAMETERS (ExportSelector.of(ImmutableDebugParametersCSV.class)),
	/** Exporter for final state data, using the ImmutableOutbreakFinalStateCSV format. */
	FINAL_STATE (ExportSelector.of(ImmutableOutbreakFinalStateCSV.class)),
	/** Exporter for behaviour count data, using the ImmutableOutbreakBehaviourCountCSV format. */
	BEHAVIOUR (ExportSelector.of(ImmutableOutbreakBehaviourCountCSV.class)),
	/** Exporter for contact count data, using the ImmutableOutbreakContactCountCSV format. */
	 ...
	 * @return the ExportSelector instance that defines how to extract and format data for this exporter
	 */
