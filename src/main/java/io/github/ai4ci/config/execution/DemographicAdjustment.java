...
public interface DemographicAdjustment {
	...
	public static interface Execution<DIST, NUMERIC> {
		...
		/**
		 * Asymptomatic fraction by age group, interpreted on the scale defined by
		 * the {@link io.github.ai4ci.config.Scale} annotation used on callers.
		 *
		...
		/**
		 * Compliance probability by age group (for example adherence to
		 * isolation).
		 *
		 ...
		 *
		 * @return a distribution-like object expressing the maximum contact
		 *         reduction
		 */
		@Scale(Scale.ScaleType.ODDS)
	...
	public static PartialDemographicAdjustment AGE_DEFAULT = PartialDemographicAdjustment
			...
			.setContactProbability(
					ImmutableEmpiricalFunction.builder().setX(0, 5, 15, 25, 45, 75)
							.setY(2, 1, 1.5, 1.25, 0.8, 1.2).setLink(LinkFunction.LOG)
							.build()
