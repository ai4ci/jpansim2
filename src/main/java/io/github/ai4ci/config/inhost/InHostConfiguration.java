 ...
 *
 * <p>
 * This interface is a common supertype for the various in‑host model
 ...
 *
 * <p>
 * Downstream uses include the initialiser
 * {@link io.github.ai4ci.flow.builders.DefaultInHostMarkovStateInitialiser}
 * which accepts an instance of this interface to build an individual's in‑host
 * state. It is also referenced by example code and tests.
 *
 * @author Rob Challen
 */
@JsonTypeInfo(
...
public interface InHostConfiguration extends Serializable {
	 ...
	 * This is used to trim the infectivity profile and the severity profile to
	 * the point where 99.9% of transmission or severity has occurred. This is
	 * not critical as long as it is high enough to capture the full profile, but
	 * not too high to be inefficient. The profiles are trimmed to the point
	 * where 99.9% of transmission or severity has occurred, so this is not
	 ...
	 *                              calculate the infectivity profile.
	 ...
	 * severity cutoffs for symptoms, hospitalisation and fatality by the
	 * downstream class {@link Calibration}.
	 *
	 ...
	 *                   profile. This is used to get the in host configuration,
	 *                   but also to get the infection case rate, hospitalisation
	 *                   rate and fatality rate to calculate the severity cutoffs
	 *                   for symptoms, hospitalisation and fatality by the
	 *                   downstream class {@link Calibration}.
	 * @param samples    the number of samples to use to get the empirical
	 *                   distribution.
	 * @param duration   the number of time steps to calculate the severity
	 *                   profile for. This should be long enough to capture the
	 *                   full profile, but not too long to be inefficient. The
	 *                   profile is trimmed to the point where 99.9% of severity
	 *                   has occurred, so this is not critical as long as it is
	 *                   long enough.
	 *
	 ...
	 * The severity profile is the average severity at each time post exposure,
	 * averaged across all agents. The profile is trimmed to the point where
	 * 99.9% of severity has occurred, so this is not critical as long as it is
	 * long enough.
	 *
	 * @param execConfig the execution configuration to use to get the severity
	 *                   profile. This is used to get the in host configuration,
	 ...
	 *                   This should be large enough to get a stable estimate of
	 *                   the profile, but not too large to be inefficient. The
	 *                   profile is trimmed to the point where 99.9% of severity
	 *                   has occurred, so this is not critical as long as it is
	 *                   large enough.
	 * @param duration   the number of time steps to calculate the severity
	 ...
	 *         exposure, averaged across all agents. The profile is trimmed to
	...
	/**
	 * For a configuration gets an average viral load profile. This is used to
	 * calibrate the transmission parameter to get the desired R0, and to
	 * calculate the infectivity profile. It is not used directly in the model,
	 * which calculates transmission from the viral load of each individual
	 * agent.
	 *
	 * This is used downstream in classes like {@link OutbreakBaseline} to
	 * calculate the transmission parameter to get the desired R0, and to
	 * calculate the infectivity profile, and in {@link Calibration} to calculate
	 * the severity cutoffs.
	 *
	 * @param execConfig the configuration to get the viral load profile for
	 * @param samples    the number of samples to use to get the average profile.
	 * @param duration   the number of time steps to calculate the profile for.
	 *                   This should be long enough to capture the full profile,
	 *                   but not too long to be inefficient. The profile is
	 *                   trimmed to the point where 99.9% of transmission has
	 *                   occurred, so this is not critical as long as it is long
	 *                   enough.
	 *
	 * @return an array with sample as first dimension and time post exposure and
	 ...
	 *
	 * <p>
	 * This is calibrated to get the desired infection fatality rate (IFR) for
	 * the outbreak. The IFR is the proportion of all infected people that get
	 * fatal outcomes, so it is the proportion of people above the cutoff.
	 *
	 * @param outbreak      the outbreak to get the severity cutoff for.
	 * @param configuration the execution configuration to get the severity
	 *                      cutoff for.
	...
	/**
	 * The cutoff is the severity level above which people get hospitalised. This
	 * is calibrated to get the desired infection hospitalisation rate (IHR) for
	 * the outbreak. The IHR is the proportion of all infected people that get
	 ...
	 * The IHR overall is 10% of the 60% symptomatic, so 6% The cutoff is the
	 * people that don;t get hospitalised so 94% quantile.
	 *
	 * @param outbreak      the outbreak to get the severity cutoff for. This is
	 *                      used to get the infection hospitalisation rate (IHR)
	 *                      to calibrate the cutoff to.
	 * @param configuration the execution configuration to get the severity
	 *                      cutoff for. This is used to
	 * @return the severity cutoff for hospitalisation. This is the severity
	 *         level above which people get hospitalised, calibrated to get the
	 *         desired infection hospitalisation rate (IHR) for the outbreak.
	 */
	 ...
	 * calibrated to get the desired infection case rate (ICR) for the outbreak.
	 * The ICR is the proportion of all infected people that get symptoms, so it
	 * is the proportion of people above the cutoff. For example, if there are
	 * lets say 40% asymptomatic, the cutoff is the 60% quantile.
	 *
	 * @param outbreak      the outbreak to get the severity cutoff for. This is
	 *                      used to get the infection case rate (ICR) to
	 *                      calibrate the cutoff to.
	 * @param configuration the execution configuration to get the severity
	 *                      cutoff for. This is used to get the infection case
	 *                      rate (ICR) to calibrate the cutoff to.
