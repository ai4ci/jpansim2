...
/**
 * A generic state machine implementation for managing transitions between different
 * behavioral and policy states in agent-based models.
 * 
 * Supports both individual behavior patterns (BehaviorState) and policy patterns (PolicyState).
 * State transitions are implemented using enumerated states that define transition logic
 * through history updates and state updates.
 * 
 * <h2>Transition Cycle</h2>
 * <p>The state machine follows a two-phase update cycle:
 * <ul>
 *   <li><b>History Update Phase</b>: Occurs early in the update cycle, handling actions
 *       like testing that need to be recorded</li>
 *   <li><b>State Update Phase</b>: Occurs at the end of the update cycle, determining
 *       the next state based on current conditions</li>
 * </ul>
 * 
...
public class StateMachine implements Serializable {
	...
	/**
	 * Updates the state machine and stores the new person state.
	 * Handles death state transitions automatically.
	 * 
	...
	public synchronized void performStateUpdate(ImmutablePersonState.Builder builder, PersonState person, Sampler rng) {
		if (person.isDead()) {
			...
			if (this.getState() instanceof State.BehaviourState) {
				State.BehaviourState state = (State.BehaviourState) this.getState();
				State.BehaviourState next = state.nextState(builder, person, context, rng);
				currentState = next;
			}
		}
	}
	...
	public synchronized void performStateUpdate(ImmutableOutbreakState.Builder builder, OutbreakState outbreak, Sampler rng) {
		if (this.getState() instanceof State.PolicyState) {
			State.PolicyState state = (State.PolicyState) this.getState();
			State.PolicyState next = state.nextState(builder, outbreak, context, rng);
			currentState = next;
		}
	}
	 ...
	 * Maintains state branching history when switching between different behavior models.
	 * 
	 * @param state the target behavior state
	 * @throws RuntimeException if attempting to update a policy state machine with a behavior state
	 */
	...
	/**
	 * Forces a transition to a specific policy state.
	 * Maintains state branching history when switching between different policy models.
	 * 
	 * @param state the target policy state
	 * @throws RuntimeException if attempting to update a behavior state machine with a policy state
	 */
	...
	/**
	 * Remembers the current state if switching between different behavior/policy models.
	 * Only remembers state branches when switching to a different model (different enum type).
	 * 
	...
	/**
	 * Returns to a previously remembered state from a branch.
	 * Appropriately handles both behavior and policy state transitions.
	 */
