...
/**
 * Utility methods for state transitions, behavior modifications, and testing logic
 * within the agent-based modeling framework.
 * 
 * <p>This class provides reusable implementations for common state machine operations
 * including mobility adjustments, testing procedures, compliance management, and
 * controlled state transitions with branching support.
 * 
 * <h2>Key Functionality Areas</h2>
 * <ul>
 *   <li><b>Testing Interfaces</b>: Default interfaces for PCR testing behaviors</li>
 *   <li><b>Mobility Control</b>: Methods for modifying agent sociability based on symptoms and compliance</li>
 *   <li><b>Behavior Restoration</b>: Gradual restoration of normal behavior after restrictions</li>
 *   <li><b>State Branching</b>: Controlled temporary state transitions with automatic return paths</li>
 *   <li><b>Testing Logic</b>: Screening and diagnostic testing with eligibility checks</li>
 * </ul>
 * 
 * @see State
 * @see StateMachine
...
public class StateUtils {
	 ...
	 * 
	 * <p>This default implementation automatically triggers PCR testing when:
	 * - Person has been symptomatic for 2 consecutive days AND
	 * - Person is compliant with testing protocols AND  
	 * - Person has not been recently tested (within disease incubation period)
	 * 
	 * <p>The timing threshold (2 days) prevents premature testing while ensuring
	 * timely intervention after symptom onset.
	 */
	public static interface DoesPCRIfSymptomatic extends State.BehaviourState {
		/**
		 * Default history update that automatically seeks PCR testing when symptomatic.
		 * 
	 ...
	 * 
	 * <p>Used for baseline behaviors or states where testing is not applicable
	 * (e.g., deceased agents, non-reactive behaviors). Provides empty implementation
	 * to satisfy interface requirements without adding testing overhead.
	 */
...
//	}
	...
	/**
	 * Applies self-isolation mobility reduction if person is compliant AND symptomatic.
	 * 
	 * <p>This is typically used for voluntary isolation scenarios where compliance
	 * determines whether the isolation measures are followed. Side effects:<ul>
	 ...
	 * 
	 * <p>Models advisory-based isolation where compliance determines adherence
	 * to recommendations rather than enforced restrictions. Used in scenarios
	 * where isolation is suggested but not mandatory.
	 * 
	 * @param builder builder for person state modifications
		 * @param person current person state to evaluate
	 */
	 ...
	 * 
	 * <p>Models enforced lock-down scenarios where mobility restrictions are
	 * mandatory rather than advisory. Sets mobility to the minimum self-isolation
	 * depth defined in the baseline configuration.
	 * 
	 * @param builder builder for person state modifications
		 * @param person current person state to evaluate
	 */
	 ...
	 * 
	 * <p>Applies continuous mobility reduction using exponential decay toward
	 * the self-isolation depth. The decay rate is controlled by the organic
	 * mobility change parameter from model configuration.
	 * 
	 * @param builder builder for person state modifications
		 * @param person current person state to evaluate
	 */
	...
	/**
	 * Gradually restores mobility and transmissibility toward baseline when asymptomatic.
	 * 
	 * <p>Used during recovery phase to transition from restricted to normal behavior.
	 * Restoration occurs only when person is asymptomatic, preventing premature
	 * restoration while infectious.
	 * 
	 * @param builder builder for person state modifications
		 * @param person current person state to evaluate
	 */
	...
	/**
	 * Unconditional gradual restoration of mobility and transmissibility to baseline.
	 * 
	 * <p>Applies exponential decay restoration toward full mobility (1.0) regardless
	 * of symptomatic status. Uses the organic mobility change rate from configuration
	 * to control restoration speed.
	 * 
	 * @param builder builder for person state modifications
		 * @param person current person state to evaluate
	 */
	 ...
	 * 
	 * <p>Immediately restores normal behavior without gradual transition. Used
	 * when forced behavior reset is needed (e.g., policy changes, state transitions).
	 * 
	 * @param builder builder for person state modifications
	...
	/**
	 * Performs diagnostic PCR test with 1-day symptom threshold. Seek a test if any symptoms (and compliant, and not recently tested)
	 * 
	 * @param builder builder for person history updates
	 * @param person current person state
	 * @param context state machine context
	 ...
	 * 
	 * <p>Used for diagnostic testing triggered by symptoms or risk exposure.
	 * Sets reactive testing flag and adds test result to person's daily history.
	 * 
	 * @param builder builder for person history updates
	 * @param person current person state
	 * @param context state machine context
	 ...
	 * 
	 * <p>Used for routine screening rather than symptom-driven diagnostics.
	 * Does not set reactive testing flag since this is not symptom-initiated.
	 * 
	 * @param builder builder for person history updates
	 * @param person current person state
	 ...
	 * 
	 * <p>Similar to PCR testing but uses LFT (Lateral Flow Test) with different
	 ...
	 * @param person current person state
	 * @param context state machine context
	 * @param days number of consecutive symptomatic days required to trigger testing
	 * 
	 */
	public static void seekPcrIfSymptomatic(ImmutablePersonHistory.Builder builder, 
	 ...
	 * 
	 * <p>Prevents excessive testing by ensuring person hasn't been tested
	 * within the disease incubation period defined in configuration.
	 * 
	 * @param person current person state to evaluate
	 ...
	 * 
	 * <p>Uses shorter restriction period (2 days) since LFTs are typically
	 * used more frequently than PCR tests.
	 * 
	 * @param person current person state to evaluate
	 ...
	 * 
	 * <p>Scans today's test results for positive outcomes. Primarily used
	 * with LFT results since they provide immediate feedback.
	 * 
	 * @param person current person state to evaluate
	 ...
	 * 
	 * <p>Reduces compliance modifier linearly toward zero using configuration
	 ...
	 * 
	 * <p>Increases compliance modifier linearly toward 1.0 using configuration
	...
	/**
	 * Creates a timed behavior state that automatically returns after i iterations.
	 * 
	 * <p>Wraps a target state to automatically revert after specified iterations.
	 ...
	 */
	public static State.BehaviourState countdown(int i, State.BehaviourState state) {
		...
		return new State.BehaviourState() {
			...
			@Override
			public State.BehaviourState nextState(io.github.ai4ci.abm.ImmutablePersonState.Builder builder,
					PersonState current, StateMachineContext context, Sampler rng) {
				State.BehaviourState out = state.nextState(builder, current, context, rng);
	 ...
	 * 
	 * <p>Applies linear restoration of mobility and transmissibility modifiers
	 ...
	 */
	public static State.BehaviourState graduallyRestoreBehaviour(int i, State.BehaviourState state) {
	 ...
	 * 
	 * <p>Selects living people with available history builders and applies
	 ...
	 */
	public static void branchPeopleTo(OutbreakState current, State.BehaviourState behaviour) {
	 ...
	 * 
	 * <p>Forces selected people to temporary behavior state while preserving
	 * current state in branch stack for later recovery. Dead people are excluded.
	 * 
	 * @param current current outbreak state
	 * @param behaviour target behavior state
	 * @param filter predicate to select which people to modify
	 */
	public static void branchPeopleTo(OutbreakState current, State.BehaviourState behaviour, Predicate<Person> filter) {
		ModelNav.people(current)
			.filter(filter)
			.filter(p -> !p.getCurrentState().isDead())
			.forEach( ps -> {
	 ...
	 * 
	 * <p>Reverses branching operations by recovering previous states from
	 * the state machine branch stack. Excludes dead people.
	 * 
	 * @param current current outbreak state
	 */
	 ...
	 * 
	 * <p>Preserves current state in branch stack and returns the target behavior
	 ...
	 */
	public static State.BehaviourState branchTo(PersonState current, State.BehaviourState behaviour) {
	 ...
	 * 
	 * <p>Used during state transitions to recover from temporary branched states.
	 * 
	 * @param context state machine context containing branch stack
	 * @return the previous behavior state from branch
	 */
	public static State.BehaviourState toLastBranchPoint(StateMachineContext context) {
