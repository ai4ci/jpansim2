 ...
 * {@link io.github.ai4ci.flow.mechanics.Updater} to create custom update stages for outbreaks and people.
 * 
 */
public class ModelOperation {

	/**
	 * Functional interfaces for the selector and consumer functions used in update stages.
	 * These are defined here to avoid dependencies on external libraries and to ensure
	 * they are serializable for use in distributed contexts.
	...
	/**
	 * TriFunction is defined for potential future use cases where a three-parameter
	 * function might be needed. It follows the same pattern as TriConsumer,
	 * ensuring it is serializable and can be used in similar contexts where a
	 ...
	 * potential future use cases where a two-parameter function might be needed. It
	 * follows the same pattern as TriFunction, ensuring it is serializable and can
	...
	/**
	 * Creates a custom updating function that runs against an outbreak when
	 * the selector is matched. The updating function can change anything about 
	 * the future state of the outbreak.
	 *  
	 ...
	 *   outbreak, and a RNG.
	 * @return An updater suitable for {@link io.github.ai4ci.flow.mechanics.Updater#withOutbreakProcessors(OutbreakStateUpdater...)}
	 */
	...
	/**
	 * Creates a custom updating function that runs against an person when
	 * the selector is matched. The updating function can change anything about 
	 * the future state of the person.
	 *  
	 * @param selector selects whether to run this updater for a given Person
	 * @param consumer a three parameter function taking the builder, current
	 *   outbreak, and a RNG.
	 * @return An updater suitable for {@link io.github.ai4ci.flow.mechanics.Updater#withPersonProcessors(PersonStateUpdater...)}
	 */
