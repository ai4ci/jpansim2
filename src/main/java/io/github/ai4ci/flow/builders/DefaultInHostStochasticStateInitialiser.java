 ...
 *
 * <p>
 * This interface constructs an {@link InHostStochasticState} from a
 ...
 *
 * <p>
 * Role in composition: the initialiser is consumed by
 * {@link io.github.ai4ci.flow.builders.DefaultPersonInitialiser} and is
 * composed into {@link io.github.ai4ci.flow.builders.DefaultModelBuilder}. The
 * model builder delegates to this interface's default implementation using
 * {@code DefaultInHostStochasticStateInitialiser.super.initialiseInHostModel(...)}.
 * Keeping this logic isolated makes it trivial to swap alternative stochastic
 * initialisers.
 *
 * <p>
 * Demographic adjustment: if a {@link PersonDemographic} is provided the
 ...
 *
 * <p>
 * Extension guidance: implementers should focus this component on creating the
 * immutable in‑host stochastic state and not on external IO; expensive
...
public interface DefaultInHostStochasticStateInitialiser {
	 ...
	 *
	 * <p>
	 * The default implementation applies demographic adjustment (if a
	 * demographic is present), copies scalar rates and samples distributional
	 * entries to populate initial target and immune pools used by the stochastic
	 * update mechanics.
	 *
	 ...
	 */
	default InHostStochasticState initialiseInHostModel(
			StochasticModel configuration, ExecutionConfiguration execConfig,
		...
		return ImmutableInHostStochasticState.builder().setTime(time)
				.setBaselineViralInfectionRate(
						configuration.getBaselineViralInfectionRate()
				)
				.setBaselineViralReplicationRate(
						configuration.getBaselineViralReplicationRate()
				...
				.setTargetExposed(0).setTargetInfected(0).setVirions(0)
				.setVirionsProduced(0).setImmunePriming(0).setImmuneActive(0)
				.setImmuneTargetRatio(
						configuration.getImmuneTargetRatio().sample(rng)
				)
				.setImmuneActivationRate(
						configuration.getImmuneActivationRate().sample(rng)
				)
				.setImmuneWaningRate(
						configuration.getImmuneWaningRate().sample(rng)
				)
				.setTargetRecoveryRate(
						configuration.getTargetRecoveryRate().sample(rng)
				)
				.setInfectionCarrierProbability(
						configuration.getInfectionCarrierProbability().sample(rng)
				).build();
	}
}