 ...
 *
 * <p>This interface provides a focussed default implementation that produces
 * the runtime {@link ImmutableOutbreakState} from a baselined {@link Outbreak}
 * and its {@link io.github.ai4ci.config.execution.ExecutionConfiguration}.
 * The default method encapsulates straightforward mappings from execution
 * configuration priors into the outbreak runtime state (for example
 * transmissibility modifiers, presumed incubation and infectious periods,
 * and screening/triggers).
 *
 * <p>Role in composition: this interface is designed to be composed into
 * higher level builders such as {@link io.github.ai4ci.flow.builders.DefaultModelBuilder}.
 * The {@code DefaultModelBuilder} implements this interface and delegates
 * outbreak initialisation to the default implementation using
 ...
 * differently or by overriding the delegation point in a subclass of the
 * model builder.
 *
 * <p>Rationale and extension points: keeping outbreak initialisation as a
 * small, composable default makes the code easier to test and extend. If a
 * simulation requires a custom outbreak initial state (for example different
 * policy parameters or additional runtime metrics), implementers should
 * either provide an alternative implementation of this interface or subclass
 ...
 * such work; prefer to perform expensive data preparation in the setup stage
 * or during baselining.
 *
 * <p>Sequencing expectations: the default implementation assumes the outbreak
 * has already been baselined (see {@link DefaultOutbreakBaseliner}) and that
...
public interface DefaultOutbreakInitialiser {
	 ...
	 *
	 * <p>The default implementation maps a small set of values from the
	 ...
	 * (for example assumed incubation and infectious durations used by
	 * detection and screening logic).
	 *
	 * <p>When composed into {@link io.github.ai4ci.flow.builders.DefaultModelBuilder}
	 * this method is invoked via explicit delegation. Implementers replacing
	 * the default should preserve the contract that the outbreak is baselined
	 * and that the returned object contains global runtime priors only; any
	 * additional computed aggregates should be added in a well documented way.
	 *
	 * @param builder builder for {@link ImmutableOutbreakState}
	 * @param outbreak the baselined outbreak used as input
	 * @param sampler a sampler available for stochastic choices during initialisation
