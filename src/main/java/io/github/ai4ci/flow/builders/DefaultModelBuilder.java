 ...
 * interfaces.
 * 
 * Extending model builders should consider whether to override behaviour by subclassing
 * or by composing alternative interfaces. For example, to change the default
 * outbreak baselining logic a builder could either implement a different baseliner
 * interface or could subclass {@code DefaultModelBuilder} and override the
 * {@code baselineOutbreak} method directly. Both approaches are valid and the
 * best choice depends on the extent of the change and the desired level of
 * modularity. The methods can coexist and the delegation to the default interfaces ensures that the
 * wiring is clear and that alternative implementations can be easily swapped in without modifying the builder itself.
 *
 * <p>Rationale: the builder composes several default behaviour interfaces
 * (for example {@link DefaultNetworkSetup}, {@link DefaultOutbreakBaseliner}
 * and {@link DefaultPersonInitialiser}) rather than centrally implementing
 * all logic. This compositional approach has several benefits:
 * <ul>
 *   <li>Separation of responsibilities — each interface encapsulates a
 *       distinct concern such as network construction or person baselining.</li>
 ...
 *       isolation.</li>
 *   <li>Extensibility — callers can subclass {@code DefaultModelBuilder}
 *       and selectively override only the hooks they need to change.</li>
 * </ul>
 *
 * <p>Implementation detail: each public method in this class delegates to
 * the corresponding default implementation on the composed interface using
 * {@code InterfaceName.super.method(...)}. This explicit delegation makes
 * the wiring obvious and preserves the ability for subclasses to override
 * behaviour either by implementing a different interface or by overriding
 * the method directly.
 *
 * <p>Composition specifics: the class composes the following default
 * interfaces in the standard build pipeline: {@link DefaultNetworkSetup},
 ...
 * specialised in‑host initialiser interfaces) and sets importation and
 * basic per‑person modifiers. To customise person initialisation replace the
 * {@code DefaultPersonInitialiser} implementation or subclass
 * {@code DefaultModelBuilder} and override the {@code initialisePerson}
 * delegation method.
 *
 * <p>Downstream users such as examples and test harnesses can use this
 * builder as a ready‑made configuration for most simulations and replace
 * individual components by subclassing or by providing alternative
	...
	DefaultPersonBaseliner, DefaultPersonInitialiser
{
	 ...
	 *
	 * <p>This method intentionally delegates to
	 ...
	 *
	 * <p>The method forwards to the default implementation so that global
	 * outbreak baseline concerns (policy defaults, aggregated metrics) are
	 * defined in a single place. Subclasses can override to provide custom
	 * baselining behaviour or to compose alternative baseline logic.
	 */
	@Override
	 ...
	 *
	 * <p>Person level baselining (demographics, household membership, persistent
	 ...
	 *
	 * <p>The initialiser converts baseline values into runtime state. By
	 * delegating to {@link DefaultPersonInitialiser} the default conversion
	 * logic (for example seeding infection timers or initial immunity) is
	 * centralised and can be updated independently of the model builder.
	 */
	@Override
	 ...
	 *
	 * <p>Outbreak runtime values that depend on the baselined outbreak are
	 * computed by {@link DefaultOutbreakInitialiser}. Delegating keeps this
	 * class focused on composition and makes it straightforward to swap in an
	 * alternative initialisation strategy.
	 */
	@Override
