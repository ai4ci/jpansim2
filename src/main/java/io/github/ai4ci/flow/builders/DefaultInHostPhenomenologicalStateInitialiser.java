 ...
 *
 * <p>This interface provides the default logic to construct an
 ...
 * viral/immune peak timings and calibrates simple biphasic logistic models
 * used to represent viral load and immune response.
 *
 * <p>Role in composition: this initialiser is composed into
 ...
 * {@code DefaultModelBuilder} delegates to this interface's default method
 * to create a person's phenomenological in‑host state. Replacing the
 * in‑host behaviour can be done by substituting this interface or by
 * overriding the delegation point in a subclass of the model builder.
 *
 * <p>Demographic adjustment: where a {@link PersonDemographic} is
 * supplied the configuration is adjusted with
 * {@link io.github.ai4ci.util.ReflectionUtils#modify} prior to sampling so
 * that age or group specific scalings provided by
 * {@link io.github.ai4ci.config.execution.DemographicAdjustment.Phenomenological}
 * are applied.
 *
 * <p>Extension guidance: keep this initialiser focused on creating the
 * immutable in‑host state and avoid heavy IO; any expensive precomputation
 * should be performed at outbreak baselining.
 *
 * @author Rob Challen
 */
public interface DefaultInHostPhenomenologicalStateInitialiser {
	 ...
	 * configuration.
	 *
	 * <p>The default implementation samples incubation, time to peak and
	 * recovery delays, and calibrates the viral load and immune response
	 * models using {@link BiPhasicLogistic} helper methods. Demographic
	 * adjustments are applied before sampling.
	 *
