 ...
 *
 * <p>This interface supplies a focussed default implementation that converts
 * a {@link MarkovStateModel} configuration and execution parameters into an
 * {@link InHostMarkovState} instance. The implementation samples durations
 * and produces per‑day transition probabilities used by the Markov in‑host
 * state machine.
 *
 * <p>Role in composition: the method defined here is intended to be used by
 ...
 * In the default composition the model builder delegates to this interface's
 * default method via
 * {@code DefaultInHostMarkovStateInitialiser.super.initialiseInHostModel(...)}.
 * This separation keeps in‑host specific logic isolated and easily
 * replaceable.
 *
 * <p>Demographic adjustment: the implementation consults the optional
 ...
 * configuration. This allows age or group specific variation in incubation
 * and symptom timings via the configured
 * {@link io.github.ai4ci.config.execution.DemographicAdjustment.Markov}.
 *
 * <p>Extension guidance: implementers that need different Markov initial
 * behaviour should replace this interface or subclass the model builder and
 * override the delegation point. Keep the initialiser responsible only for
 * creating the initial immutable in‑host state; avoid performing heavy IO
 * here unless managed by the calling builder.
 *
 * @author Rob Challen
 */
public interface DefaultInHostMarkovStateInitialiser {
	 ...
	 *
	 * <p>The default implementation applies demographic adjustments (if a
	 ...
	 * @param execConfig the execution configuration providing population rates
	 * @param person optional person demographic used for demographic adjustment
