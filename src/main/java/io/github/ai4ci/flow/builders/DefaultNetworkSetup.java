 ...
 * options supplied. This is config driven by {@link io.github.ai4ci.config.setup.NetworkConfiguration}. In 
 * the second stage it determines the weight of the social network 
...
public interface DefaultNetworkSetup {
	 ...
	 * network configuration supplied. The network is built using JGraphT and then
	 * converted to a thread safe array of social relationships. The relationship
	 * strengths are determined by the demographic configuration. The resulting
	 * social network is set on the outbreak and the outbreak is returned for
	 * further processing by other setup functions.
	 * 
	 * @param outbreak the outbreak to set up
	 * @param setupConfig the setup configuration containing network and demographic settings
	 * @param sampler a sampler for generating random numbers used in network construction and relationship strength assignment
	...
	default ModifiableOutbreak setupOutbreak(ModifiableOutbreak outbreak, SetupConfiguration setupConfig, Sampler sampler) {
		...
		log.debug("contact graph {} edges, {} average degree ", 
				...
				((double) outbreak.getSocialNetwork().size() * 2.0) / outbreak.getPopulationSize()
			);
		return outbreak;
	}
