 ...
 *
 * <p>Main purpose: record the infectivity profile used by the outbreak baseline
 * as a time indexed distribution. Records are produced during the BASELINE
 * stage and exported per model to a CSV file ({@code ip.csv}).
 *
 * <p>Downstream uses: used by plotting utilities, analytic pipelines and any
 * code that reconstructs per‑time infectivity curves for sensitivity analysis.
 *
 * @author Rob Challen
 */
@Value.Immutable
...
public interface InfectivityProfileCSV extends CommonCSV.Model {
	...
	/**
	 * Selector class implements Export.Selector to provide infectivity profile extraction
	 * from Outbreak simulations. Retrieves infectivity profile data using CSVMapper
	 ...
	 * are typically normalised so that the profile integrates to the baseline
	 * infectiousness over the infectious period
	 */
