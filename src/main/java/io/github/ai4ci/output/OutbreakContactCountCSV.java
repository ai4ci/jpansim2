 ...
 *
 * <p>
 * Main purpose: provide counts of contacts stratified by policy or bucket;
 ...
 *
 * <p>
 * Downstream uses: useful for assessing contact distributions across policies
 * and for calibration diagnostics.
 *
 * @author Rob Challen
 */
@Value.Immutable
@Export(
		stage = Stage.UPDATE,
		value = "contact-counts.csv",
		size = 16,
		selector = OutbreakContactCountCSV.Selector.class,
		writer = CSVWriter.class
)
public interface OutbreakContactCountCSV extends CommonCSV.State {

	/**
	 * Selector for extracting contact count distribution data from an
	 * {@link Outbreak}
	 */
