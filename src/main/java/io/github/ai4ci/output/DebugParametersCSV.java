 ...
 *
 * <p>
 * Main purpose: capture key model parameters and derived diagnostics used for
 * debugging and sensitivity analysis. Records are produced during the START
 * stage and are exported per simulation execution to CSV files
 * ({@code debug.csv}).
 *
 * <p>
 * Downstream uses: consumed by diagnostic pipelines, regression tests and
...
@Export(
		...
		value = "debug.csv",
		size = 64,
		selector = DebugParametersCSV.Selector.class,
		writer = CSVWriter.class
)
public interface DebugParametersCSV extends CommonCSV.Execution {

	/**
	 * Selector class implements Export.Selector to provide debug parameter
	 * extraction from Outbreak simulations. Transforms baseline configuration
	 * data into parameter records using CSVMapper for CSV serialization.
	 */
	 ...
	 *
	 * @return the average number of contacts per agent used as a diagnostic for
	 *         network sparsity and mixing assumptions
	 */
