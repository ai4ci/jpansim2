 ...
 * {@link #setupOutbreak(ModifiableOutbreak, SetupConfiguration, Sampler)} and
 ...
 * {@link io.github.ai4ci.config.setup.SetupConfiguration#hilbertBits} or
 * {@link io.github.ai4ci.config.setup.SetupConfiguration#getHilbertCoords(Integer)}
 ...
 * {@link io.github.ai4ci.util.ThreadSafeArray}. Implementers that modify shared
...
public abstract class AbstractModelBuilder {
	 ...
	 * degree distributions) here to speed up per‑person baselining.
	 *
	 ...
	 */
	public abstract ImmutableOutbreakBaseline baselineOutbreak(
	 ...
	 * initialisation can be precomputed here and stored in the baseline object.
	 *
	 ...
	 */
	public abstract ImmutablePersonBaseline baselinePerson(
	 ...
	 * baselining can remain lightweight and possibly parallel.
	 *
	 ...
	 */
	public ImmutableOutbreakBaseline doBaselineOutbreak(
		...
		if (o instanceof ModifiableOutbreak) {
			...
			if (!m.getPeople().isEmpty() && m.initialisedSetupConfiguration()
					&& m.initialisedExecutionConfiguration()
					&& m.initialisedSocialNetwork()
					&& this.isBaselined(m.getPeople()))
				return this.baselineOutbreak(builder, o, sampler);
		}
		throw new RuntimeException(
				"Outbreak is not configured correctly for baselining."
		);
	}
	 ...
	 * {@link #baselinePerson(ImmutablePersonBaseline.Builder, Person, Sampler)}
	 ...
	 * attached {@link SetupConfiguration} to derive individual demographic
	 ...
	 * @throws RuntimeException if preconditions for baselining are not met
	 */
	public ImmutablePersonBaseline doBaselinePerson(
		...
		if (p.getOutbreak() instanceof ModifiableOutbreak) {
			...
			if (m.initialisedSetupConfiguration()
					&& m.initialisedExecutionConfiguration()
					&& m.initialisedSocialNetwork())
				return this.baselinePerson(builder, p, sampler);
		}
		throw new RuntimeException(
				"Person is not configured correctly for baselining."
		);
	}
	 ...
	 * incorporate global modifiers derived from {@link SetupConfiguration} (for
	 * example network scaling factors or demographic aggregates) which are
	 * typically computed during the baselining stage.
	 *
	 ...
	 */
	public ImmutableOutbreakState doInitialiseOutbreak(
			ImmutableOutbreakState.Builder builder, Outbreak o, Sampler sampler
	) {
		if (o instanceof ModifiableOutbreak) {
			ModifiableOutbreak m = (ModifiableOutbreak) o;
			if (m.initialisedBaseline() && m.initialisedSetupConfiguration()
					&& m.initialisedExecutionConfiguration()
					&& m.initialisedSocialNetwork())
				return this.initialiseOutbreak(builder, o, sampler);
		}
		throw new RuntimeException(
				"Outbreak is not configured correctly for initialisation."
		);
	}
	 ...
	 * @throws RuntimeException if preconditions for initialisation are not met
	 */
	public ImmutablePersonState doInitialisePerson(
			ImmutablePersonState.Builder builder, Person p, Sampler sampler
	) {
		if (p instanceof ModifiablePerson) {
			ModifiablePerson mp = (ModifiablePerson) p;
			if (p.getOutbreak() instanceof ModifiableOutbreak) {
				ModifiableOutbreak m = (ModifiableOutbreak) p.getOutbreak();
				if (mp.initialisedBaseline() && m.initialisedSetupConfiguration()
						&& m.initialisedExecutionConfiguration()
						&& m.initialisedSocialNetwork())
					return this.initialisePerson(builder, p, sampler);
			}
		}
		throw new RuntimeException(
				"Person is not configured correctly for initialisation."
		);
	}
	 ...
	 * {@link SetupConfiguration} on the outbreak, allocates the thread safe
	 * people container sized by the configured population, and then delegates to
	 * {@link #setupOutbreak(ModifiableOutbreak, SetupConfiguration, Sampler)}
	 ...
	 * performed inside the {@link #setupOutbreak} hook where callers can control
	 ...
	 */
	public ModifiableOutbreak doSetupOutbreak(
			ModifiableOutbreak outbreak, SetupConfiguration config, Sampler sampler
	) {
		if (config == null) throw new RuntimeException(
				"Outbreak is not configured correctly for setup."
		);
		outbreak.setSetupConfiguration(config);
		outbreak.setPeople(
				new ThreadSafeArray<>(Person.class, outbreak.getPopulationSize())
		);
		return this.setupOutbreak(outbreak, config, sampler);
	}
	 ...
	 * @return the initial outbreak state
	 */
	public abstract ImmutableOutbreakState initialiseOutbreak(
	 ...
	 * {@code ImmutablePersonState} and let the caller attach it to the outbreak
	 * container.
	 *
	 ...
	 * @return the initial person state
	 */
	public abstract ImmutablePersonState initialisePerson(
	 ...
	 *
	 * <p>
	 * Note: this is an implementation detail and is intentionally private.
	 */
	 ...
	 * to census data based on {@link SetupConfiguration#getDemographics()}.</li>
	 ...
	 * {@link SetupConfiguration#getInitialImports()} and any configured seeding
	 * strategy.</li>
	 * </ul>
	 *
	 ...
	 */
	public abstract ModifiableOutbreak setupOutbreak(
			ModifiableOutbreak outbreak, SetupConfiguration config, Sampler sampler
	);
