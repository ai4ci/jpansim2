 ...
 *
 * <p>
 * This interface provides a focused default implementation for person
 ...
 * <p>
 * Role in composition:
 * {@link io.github.ai4ci.flow.builders.DefaultModelBuilder} implements this
 * interface and delegates person baselining to the default method via
 ...
 *
 * <p>
 * Behaviour and sequencing: the default method adjusts the outbreak's execution
 * configuration for the individual's demographic group (using
 ...
 * person state; in particular it assumes the outbreak's execution configuration
 * and demographic adjustment are available.
 *
 * <p>
 * Extension guidance: implementers who replace this default should preserve the
 * contract that the method returns a built {@link ImmutablePersonBaseline} and
 * should avoid heavy IO or long running tasks inside the method unless
 * coordinated by the calling builder. For compute heavy per‑person work
 * consider precomputing aggregates during outbreak baselining.
 *
 * @author Rob Challen
 */
public interface DefaultPersonBaseliner {
	 ...
	 *
	 * <p>
	 * The default implementation performs these steps:
	 * <ol>
	 * <li>Adjust the outbreak execution configuration using the person's
	 * demographic via {@link ReflectionUtils#modify}.</li>
	 * <li>Initialise the person's state machine with the configured default
	 * behaviour model.</li>
	 ...
	 * <li>Set symptom sensitivity, specificity and self‑isolation depth from the
	 * adjusted configuration.</li>
	 * </ol>
	 *
	 * <p>
	 * Composition note: when used from
	 * {@link io.github.ai4ci.flow.builders.DefaultModelBuilder} this method is
	 * invoked by the builder's delegation and therefore forms the canonical
	 * per‑person baseline behaviour for the default builder.
	 *
	 ...
	 */
	default ImmutablePersonBaseline baselinePerson(
			ImmutablePersonBaseline.Builder builder, Person person, Sampler rng
	) {
		ExecutionConfiguration configuration = ReflectionUtils.modify(
		...
		builder
				.setMobilityBaseline(
						configuration.getContactProbability().sample(rng)
				)
				.setComplianceBaseline(
						configuration.getComplianceProbability().sample(rng)
				)
				.setAppUseProbability(
						configuration.getAppUseProbability().sample(rng)
				).setDefaultBehaviourState(configuration.getDefaultBehaviourModel())

				.setSymptomSensitivity(
						configuration.getSymptomSensitivity().sample(rng)
				)
				.setSymptomSpecificity(
						configuration.getSymptomSpecificity().sample(rng)
				).setSelfIsolationDepth(
						configuration.getMaximumSocialContactReduction().sample(rng)
				);
