...
/**
 * A linear interpolator implementation that performs piecewise linear
 * interpolation between given data points.
 * <p>
...
public class LinearInterpolator implements Interpolator {
	 ...
	 *
	 * @param x the x-coordinates of the data points (must be strictly
	 *          increasing)
	 * @param y the y-coordinates of the data points
	 * @return a new LinearInterpolator instance
	 * @throws IllegalArgumentException if arrays have different lengths, x is
	 *                                  not strictly increasing, or arrays have
	...
	/**
	 * Computes the derivative (slope) of the interpolated function at the given
	 * x-coordinate. Returns 0.0 for values outside the interpolation range.
	 *
	...
	public double differential(double x) {
		if ((x <= this.x[0]) || (x >= this.x[this.x.length - 1])) return 0.0;
	...
	/**
	 * Interpolates the value at the given x-coordinate using linear
	 * interpolation.
	 *
