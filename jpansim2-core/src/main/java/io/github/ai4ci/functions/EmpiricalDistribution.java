...
/**
 * Empirical probability distribution constructed from sample data or from x /
 * F(x) pairs.
 *
 * <p>
 * Main purpose: provide cumulative, density, quantile and sampling
 * functionality for empirically observed variables. The implementation fits a
 * monotonic spline on a transformed scale and exposes convenient accessors for
 * mean, median, density and random sampling.
 *
 * <p>
 * Downstream uses: consumed by in-host configuration and calibration routines
 * (for example {@link io.github.ai4ci.config.inhost.InHostConfiguration} and
 * {@link io.github.ai4ci.abm.Calibration}) and by initialisers that need
...
public interface EmpiricalDistribution extends Distribution, Serializable {
	 ...
	 * points exceeds this value, the data will be downsampled to this number of
	 * points to ensure a smooth fit and to avoid overfitting. If the number of
	 * data points is less than or equal to this value, the data will be used as
	 ...
	 *
	 * @param link the link function applied to the sample values prior to
	 *             fitting
	...
	public static ImmutableEmpiricalDistribution fromData(
		...
		if (data.length > KNOTS) {
			...
			var step = ((float) data.length) / KNOTS;
			var x = new double[KNOTS - 1];
			var y = new double[KNOTS - 1];
	 ...
	 *
	 * @param odds the odds-like empirical function to baseline against this
	 *             density
...
//          Math.min(this.getMaximum(), odds.getMaximum())
	...
	/**
	 * Compute the density at the supplied x using the derivative of the fitted
	 * spline and the link function.
	 *
	...
	/**
	 * The quantile function is the inverse of the CDF. By fitting the spline on
	 * the logit-transformed cumulative probabilities, we can easily derive the
	 * quantile function by inverting the spline and applying the inverse link
	 * function.
	 *
	 ...
	 *
	 * @return a spline interpolator representing logit(F(X)) as a function of
	 *         link(X)
	 */
	 ...
	 */
	@Override @Value.Derived @JsonIgnore
	default double getMaxSupport() { return this.getMaximum(); }
	 ...
	 */
	@JsonIgnore
	default double getMedian() { return this.getQuantile(0.5); }
	 ...
	 */
	@Override @Value.Derived @JsonIgnore
	default double getMinSupport() { return this.getMinimum(); }
	...
	/**
	 * Sample a random value from the empirical distribution using the default
	 * sampler.
	 *
