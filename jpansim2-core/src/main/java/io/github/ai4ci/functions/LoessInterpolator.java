 ...
 *
 * <p>
 * Main purpose: provide a convenient, immutable wrapper around Apache Commons
 * Math's LOESS implementation that fulfils the {@link Interpolator} contract.
 * The implementation fits a smooth piecewise polynomial to the supplied (x,y)
 * data and exposes interpolation and derivative evaluation.
 *
 * <p>
 * Key features:
 * <ul>
 ...
 *
 * <p>
 * Downstream uses: consumed wherever a smooth local regression is required for
 * numeric curves, for example in smoothing empirical profiles before fitting or
 * calibration routines (see {@link EmpiricalDistribution}).
 *
 * @author Rob Challen
 */
@Value.Immutable
public interface LoessInterpolator extends Interpolator {
	 ...
	 */
	public static LoessInterpolator createLoessInterpolator(
			double[] x, double[] y
	) {
	...
	default double differential(double x) {
		x = Interpolator.squish(this.getMinSupport(), x, this.getMaxSupport());
		return this.getLoess().derivative().value(x);
	}

	/**
	 * Returns an evaluated polynomial spline function produced by the LOESS
	 * algorithm.
	 *
	 * <p>
	 * Clients may use the returned {@link PolynomialSplineFunction} for advanced
	 * evaluation, for example to obtain higher order derivatives.
	 *
	 * @return the fitted {@link PolynomialSplineFunction}
	 */
	@Value.Lazy
	default PolynomialSplineFunction getLoess() {
		int n = this.getX().length;
		double bw = 4.0 / n;
		org.apache.commons.math3.analysis.interpolation.LoessInterpolator tmp = new org.apache.commons.math3.analysis.interpolation.LoessInterpolator(
				bw,
				org.apache.commons.math3.analysis.interpolation.LoessInterpolator.DEFAULT_ROBUSTNESS_ITERS
		);
		return tmp.interpolate(this.getX(), this.getY());
	}
	...
	default double getMaxSupport() {
		return Arrays.stream(this.getX()).max().getAsDouble();
	}
	...
	default double getMinSupport() {
		return Arrays.stream(this.getX()).min().getAsDouble();
	}

	/**
	 * The independent coordinate values used to fit the LOESS model.
	 *
	 * @return the x-coordinates (must be strictly increasing and match
	 *         {@link #getY()})
	 */
	double[] getX();
	 ...
	 *
	 * <p>
	 * Values outside the fitted support are clamped to the support boundaries to
