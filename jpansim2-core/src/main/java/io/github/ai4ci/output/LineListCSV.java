...
/**
 * Represents a line list CSV record for epidemic simulation output. This
 * interface defines the structure of individual-level data exported from an
 * outbreak simulation, including person identifiers, health status, behavioural
 * metrics, and epidemiological parameters.
 *
 * <p>
 * Records are exported during the UPDATE stage and are written per agent to a
 * DuckDB database ({@code linelist.duckdb}).
 *
 * <p>
 * Downstream uses: case line lists are typically consumed by analysis
...
@Export(
		stage = Stage.UPDATE,
		value = "linelist.duckdb",
		size = 64 * 64,
		selector = LineListCSV.Selector.class,
...
public interface LineListCSV extends CommonCSV.State {
	 ...
	 *
	 * @return true if this is the first time the person is recorded as requiring
	 *         hospitalisation
	 */
