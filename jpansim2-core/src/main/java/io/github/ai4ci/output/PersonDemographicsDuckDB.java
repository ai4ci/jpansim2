 ...
 *
 * <p>
 * Main purpose: provide per‑agent demographic and baseline attributes such as
 * age, mobility baseline and location. Records are produced at the START stage
 * and are written per agent to a DuckDB database ({@code demog.duckdb}).
 *
 * <p>
 * Downstream uses: used for descriptive statistics, stratified analyses and as
 * a join table for other per‑agent outputs such as line lists.
 *
 * @author Rob Challen
 */
@Value.Immutable
@Export(
		...
		value = "demog.duckdb",
		size = 16,
		selector = PersonDemographicsDuckDB.Selector.class,
		writer = DuckDBWriter.class
)
public interface PersonDemographicsDuckDB extends CommonCSV.Execution {

	/**
	 * Selector for person demographic records. This is used to extract the
	 * relevant data from the outbreak at the START stage and map it to
	 * PersonDemographicsCSV records for export.
	 */
	static class Selector implements Export.Selector {
		@Override
		public Stream<PersonDemographicsDuckDB> apply(Outbreak o) {
	 ...
	 *
	 * @return the normalised X location used for spatial analysis and joining to
	 *         raster or spatial models
	 */
	double getLocationX();
	 ...
	 *
	 * @return the normalised Y location used for spatial analysis and joining to
	 *         raster or spatial models
	 */
	double getLocationY();
