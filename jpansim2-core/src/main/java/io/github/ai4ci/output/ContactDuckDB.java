 ...
 *
 * <p>
 * Main purpose: represent individual contact events detected or inferred during
 * an update step of the simulation. Records are produced in the UPDATE stage
 * and are exported per agent to a DuckDB database file
 * ({@code contacts.duckdb}).
 *
 * <p>
 * Downstream uses: consumed by contact tracing analyses, contact network
...
@Export(
		stage = Stage.UPDATE,
		value = "contacts.duckdb",
		size = 64 * 64,
		selector = ContactDuckDB.Selector.class,
		writer = DuckDBWriter.class
)
public interface ContactDuckDB extends CommonCSV.State {

	/**
	 * Selector class implements Export.Selector to provide contact data
	 * extraction from Outbreak simulations. Transforms person data into contact
	 * records using CSVMapper for CSV serialization.
	 */
	static class Selector implements Export.Selector {
		@Override
		public Stream<ContactDuckDB> apply(Outbreak o) {
