...
/**
 * A behaviour model that works with
 * {@link io.github.ai4ci.abm.policy.ReactiveLockdown}, implementing policy
 * changes by allowing people to enter a low activity state.
 */
public enum LockdownIsolation implements BehaviourModel, DoesPCRIfSymptomatic {

	/**
	 * Everyone enters minimal mobility state immediately in instruction to lock
	 * down (@see {@link WAIT}). This is triggered by
	 ...
	 * randomly become non-compliant to lockdown. <!-- (@see
	 * {@link NonCompliant#DEFAULT}). --> In the case of becoming non compliant
	 * to lockdown the person will return to whatever they were doing before
	 * lockdown (but with reduced compliance)
	 */
	WAIT {
		@Override
		public State.BehaviourState nextState(
	 ...
	 * {@link io.github.ai4ci.abm.policy.ReactiveLockdown#LOCKDOWN}, people
	 * return to a mobility state larger than their baseline, gradually return to
	 * their default mobility over the next 10 days, and return to whatever state
	 * they were in before the lockdown was triggered.
	 */
	RELEASE {
		@Override
		public State.BehaviourState nextState(
