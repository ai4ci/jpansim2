...
public interface Contact extends Serializable {
	...
	default int getParticipant(int id) {
		if (id == this.getParticipant1Id()) { return this.getParticipant2Id(); }
		if (id == this.getParticipant2Id()) { return this.getParticipant1Id(); }
	 ...
	 *
	 * @param one the history of one participant in the contact, used to resolve
	 *            the other participant and the time of the contact
	 * @return the most recent history of the other participant in the contact,
	 *         as of the time of the contact
	 */
	default PersonHistory getParticipant(PersonHistory one) {
		var id = this.getParticipant(one.getEntity().getId());
	 ...
	 * Get the most recent history of the other participant in a contact
	 *
	 * @param one the state of one participant in the contact, used to resolve
	 *            the other participant and the time of the contact
	 * @return the most recent history of the other participant in the contact,
	 *         as of the time of the contact. Note that this is not necessarily
	 *         the same as the current history of the other participant, since
	 *         the contact may have occurred in the past. The current history is
	 *         returned if the contact occurred at the current time step.
	 *
	 */
	default PersonHistory getParticipant(PersonState one) {
		var id = this.getParticipant(one.getEntity().getId());
		return one.getEntity().getOutbreak().getPersonById(id)
				.flatMap(p -> p.getCurrentHistory()).get();
	}
...
//	double getProximityDuration();
	 ...
	 *
	 * @return the person id of the first participant; corresponds to the
	 ...
	 * @param one the state of one participant in the contact, used to resolve
	 *            the other participant and the time of the contact
	 * @return the current state of the other participant in the contact. Note
	 *         that this is not necessarily the same as the state of the other
	...
	default PersonState getParticipantState(PersonTemporalState one) {
		var id = this.getParticipant(one.getEntity().getId());
		return one.getEntity().getOutbreak().getPersonById(id)
				.map(p -> p.getCurrentState()).get();
	}

	/**
	 * Whether the contact was detected by a smart agent. This is used to
	 * determine whether the contact should be included in contact tracing
	 * analyses and whether it should be included in contact network statistics.
	 * The contact may have been detected by a smart agent if either participant
	 * is a smart agent and the contact was detected by that agent. Note that a
	 * contact may be detected by one agent but not the other, so this is not
