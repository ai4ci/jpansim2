...
public class InterfaceSchemaGenerator {
	...
	private static List<String> generateIfaceLines(
			Class<?> cls, Map<String, Class<?>> classByName
		...
		lines.add(
				simpleName.equals(
						"Manifest"
				) ? "# WARNING: Possible name conflict with java.util.jar.Manifest"
						: ""
		);
		lines.add(
				simpleName.equals(
						"Handler"
				) ? "# WARNING: Possible name conflict with nonapi.io.github.classgraph.classloader.Handler"
						: ""
		);

		lines.add(
				simpleName.equals("Manifest") || simpleName.equals("Handler")
		...
		for (Method method : builderClz.getMethods()) {
			if (method.isAnnotationPresent(JsonProperty.class)) {
				...
				String rType;
				String doc = "maps to java field " + simpleName + "."
						+ method.getName();
				...
				if (method.getName()
						.equals("s" + idMethod.getName().substring(1))) {
					rType += " + unique_id";
				}
				...
				fieldLines.add(
						String.format("  %s = %s ~ \"%s\"", fieldName, rType, doc)
				);
			}
		}

		// Join fields with comma
		if (!fieldLines.isEmpty()) { lines.add(String.join(",\n", fieldLines)); }

		lines.add(")");

		// Filter out empty warnings
		return lines.stream()
				.filter(s -> !s.trim().isEmpty() || s.contains("WARNING"))
				.collect(Collectors.toList());
	}

	/**
	 * Scans the given package for @Import-annotated interfaces and generates R
	 * iface() code.
	 *
	 * @throws SecurityException     if a security manager is present and denies
	 *                               access to class information
	 * @throws NoSuchMethodException if a required method (like builder()) is not
	 *                               found in the immutable class
	 * @param packageName the Java package to scan for @Import-annotated
	 *                    interfaces
	 * @param outputPath  the file path where the generated R code will be
	 *                    written
	 * @throws IOException if an I/O error occurs writing to the output file
	 *
	 */
	public static void generateSchemas(String packageName, Path outputPath)
			throws IOException, NoSuchMethodException, SecurityException {
		List<Class<?>> importClasses = scanForImportAnnotatedClasses(packageName);

		if (importClasses.isEmpty()) throw new IllegalArgumentException(
				"No @Import-annotated classes found in package: " + packageName
		);
	...
	/**
	 * Main method to execute the schema generation.
	 *
	 * @param args command line arguments: [0] package name to scan, [1] output R
	 *             file path
	 */
	public static void main(String[] args) {
		...
		try {
			...
			System.out
					.println("✅ Successfully generated R schemas at: " + outputPath);
		} catch (Exception e) {
	...
	private static List<Class<?>> scanForImportAnnotatedClasses(
			String packageName
	) {
		try (ScanResult scanResult = new ClassGraph().enableAllInfo()
			...
			return scanResult.getClassesWithAnnotation(Import.class.getName())
					.stream().filter(ClassInfo::isInterface)
