 ...
 *
 * <p>
 * Concrete implementations provide parameters for specific graph generation
 * algorithms (for example Erdos‑Reyni, Watts‑Strogatz and Barabasi‑Albert). The
 * configuration objects are consumed by the model builder during the
 * {@code setupOutbreak} stage to create the social network used by the
 * simulation.
 *
 * <p>
 * Extension guidance: to add a new network type implement a new configuration
 * class and add it to the Jackson polymorphic mapping above (or register it
 * with your object mapper). Keep generation logic in the configuration's
 * {@link #generateGraph} method so the builder can remain generic.
 * </p>
 *
 * @author Rob Challen
 */
...
public interface NetworkConfiguration extends Serializable {
	 ...
	 *
	 * <p>
	 * Builders pass an empty {@link SimpleGraph} and expect the method to
	 * populate it with nodes and edges. Implementations should add nodes and
	 * edges deterministically from the configuration values so results can be
	 * reproduced when the same seed is used.
	 *
