 ...
 * and if so provides a mechanism to subset the overall configuration depending
 * on the number of SLURM nodes involved. This means that the instance of
 * JPanSim2 running on one SLURM node does not repeat the work done by others.
 * It also prevents SLURM nodes from overwriting each others outputs by
 * providing node specific output directories.
 */
@Value.Immutable
...
public interface BatchConfiguration {
	 ...
	 *
	 * <p>
	 * The default values provide a compact set of parameters used as a baseline
	 * in examples and tests. Downstream callers that rely on this simple default
	 * include example experiments.
	 *
	...
	/**
	 * Gets the batch name, which is a combination of the batch number and total
	 * number of batches. This is useful for identifying the specific batch job
	 * when running multiple batches in parallel.
	 *
	 * @return The batch name in the format "batchNumber/batchTotal".
	 */
	@JsonIgnore @Value.Derived
	default String getBatchName() {
		return this.getBatchNumber() + "/" + this.getBatchTotal();
	}
	 ...
	 *
	 * @return The batch number, which is the index of the current job in the
	 *         SLURM array. If not running in a SLURM batch job, it defaults to
	 *         1.
	 */
	@JsonIgnore @Value.Derived
	 ...
	 * variable. Only set this if you know what you are doing
	 *
	 * @return The total number of batches, which is the total number of jobs in
	 *         the SLURM array. If not running in a SLURM batch job, it defaults
	 *         to 1.
	 */
	@JsonIgnore @Value.Derived
	 ...
	 */
	@Value.Default
	default Exporters[] getExporters() {
		return new Exporters[] { Exporters.DEMOGRAPHICS, Exporters.SUMMARY,
				Exporters.INFECTIVITY_PROFILE, Exporters.LINELIST,
	 ...
	 *         indicating that the code is running as part of a SLURM batch job;
	 *         false otherwise.
	 */
	@JsonIgnore @Value.Derived
	default boolean isSlurmBatch() {
