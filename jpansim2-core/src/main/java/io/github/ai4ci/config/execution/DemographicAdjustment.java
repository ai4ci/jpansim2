 ...
 *
 * <p>
 * Main purpose: provide a small set of interfaces that describe how various
 ...
 *
 * <p>
 * Downstream uses: referenced by
 * {@link io.github.ai4ci.config.execution.ExecutionConfiguration}, in-host
 * configuration types such as
 * {@link io.github.ai4ci.config.inhost.PhenomenologicalModel} and
 * {@link io.github.ai4ci.config.inhost.MarkovStateModel}, and by initialisers
 * in {@link io.github.ai4ci.flow.builders.DefaultPersonInitialiser}.
 *
 * @author Rob Challen
 */
public interface DemographicAdjustment {
	...
	public interface _PartialDemographicAdjustment extends
		 ...
		 */
		default _PartialDemographicAdjustment self() {
			return this;
		}
	}
	 ...
	 *
	 * <p>
	 * Implementations supply either distribution-like objects or numeric
	 * transforms that are interpreted by execution code to produce age-specific
	 * parameters such as case rates and contact odds.
	 *
	 * @param <DIST>    the type used to represent distribution-like adjustments
	 *                  (for example a function or empirical distribution)
	 * @param <NUMERIC> the type used to represent numeric adjustments (for
	...
	public static interface Execution<DIST, NUMERIC> {
		 ...
		 */
		@Scale(Scale.ScaleType.ODDS)
		DIST getAppUseProbability();

		/**
		 * A function returning an age dependent odds ratio adjusting baseline
		 * adjusting the asymptomatic fraction by age group.
		 *
		 * @return A function returning an odds ratio adjusting the asymptomatic
		 ...
		 *
		 * @return A function returning an age dependent odds ratio adjusting
		 *         baseline fatality rate
		 */
		@Scale(Scale.ScaleType.ODDS)
		 ...
		 *
		 * @return A function returning an age dependent odds ratio adjusting
		 *         baseline compliance probability
		 */
		@Scale(Scale.ScaleType.ODDS)
		 ...
		 *
		 * @return A function returning an age dependent odds ratio adjusting
		 *         baseline modify contact odds
		 */
		@Scale(Scale.ScaleType.ODDS)
		DIST getContactProbability();

		/**
		 * A function returning an age dependent odds ratio adjusting baseline
		 * maximum social contact reduction achievable under behavioural
		 * interventions.
		 *
		 * @return A function returning an age dependent odds ratio adjusting
		 *         baseline maximum contact reduction
		 */
		@Scale(Scale.ScaleType.ODDS)
	 ...
	 *
	 * <p>
	 * These entries are used by Markov-style in-host implementations to
	...
	public static interface Markov<DIST, NUMERIC> {
		 ...
		 *
		 * @return A function returning an age dependent multiplicative factor
		 *         immune waning half-life
		 */
		@Scale(Scale.ScaleType.FACTOR)
		 ...
		 *
		 * @return A function returning an age dependent multiplicative factor
		 *         applied to infectious duration
		 */
		@Scale(Scale.ScaleType.FACTOR)
	 ...
	 *
	 * <p>
	 * These entries adjust time-scales used by phenomenological in-host models
	...
	public static interface Phenomenological<DIST, NUMERIC> {
		 ...
		 *
		 * @return A function returning an age dependent multiplicative factor
		 *         applied to immune waning half-life
		 */
		@Scale(Scale.ScaleType.FACTOR)
		DIST getImmuneWaningHalfLife();
		 ...
		 *
		 * @return A function returning an age dependent multiplicative factor
		 *         applied to peak-to-recovery delay
		 */
		@Scale(Scale.ScaleType.FACTOR)
		DIST getPeakToRecoveryDelay();
	}
	...
	/**
	 * A sensible default set of age-dependent adjustments used in examples and
	 * tests.
	 *
	 * <p>
	 * This partial provides example functions for contact probability, clinical
	...
	public static PartialDemographicAdjustment AGE_DEFAULT = PartialDemographicAdjustment
		...
		.setCaseFatalityRate(
			ImmutableMathematicalFunction.builder()
				...
				// this is log odds as expressed here:
				.setFXExpression("(x-45)/10*lg(2)")
				.setLink(LinkFunction.LOG)
				.setMinimum(0)
				.setMaximum(120)
				.build()
		...
		.setAsymptomaticFraction(
			ImmutableMathematicalFunction.builder()
				// baseline 45, odds halves every additional 20 years of age
				// this is log odds as expressed here:
				.setFXExpression("-(x-45)/20*lg(2)")
				.setLink(LinkFunction.LOG)
				.setMinimum(0)
				.setMaximum(120)
				.build()
