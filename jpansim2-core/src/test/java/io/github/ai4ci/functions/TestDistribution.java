...
class TestDistribution {
	...
	static EmpiricalDistribution testAgeDist = ImmutableEmpiricalDistribution
		.builder()
		.setMinimum(0)
		.setMaximum(120)
		.setX(18, 45, 65, 85)
	...
	void test() {
		System.out.println(
			SimpleDistribution.unimodalBeta(0.99, 0.01)
				.sample()
		);
	}
	...
	@Test
	void testConversion() {
		System.out.println(Conversions.probabilityFromRate(1.0 / 10));
	}
	...
	void testEmpirical() {
		...
		IntStream.range(0, 120)
			.mapToObj(i -> dist.getDensity(i) + " " + dist.getCumulative(i))
		...
		// System.out.println(IntStream.range(0, 120).mapToDouble(i ->
		// dist.getDensity(i)).sum());
	}

	@Test
	void testResample() {

		var dist = SimpleDistribution.gamma(5D, 2D)
			.combine(SimpleDistribution.beta(0.5, 0.2), (d1, d2) -> d1 + d2);

		System.out.println(dist.getMean());
		System.out.println(dist.sample());
		System.out.println(dist.getMedian());
		System.out.println(
			dist.getInterpolation()
				.getMean()
		);
		System.out.println(
			dist.getInterpolation()
	...
	@Test
	void testInterp() {
		var fn = ImmutableEmpiricalFunction.builder()
			.setX(0, 10, 25, 40, 60, 70)
			.setY(2, 0.5, 1.5, 0.5, 1.0, 0.5)
			.build();

		var testAgeDiffDist = testAgeDist
			.combine(testAgeDist, (d1, d2) -> Math.abs(d1 - d2))
			.getInterpolation();

		var fn2 = testAgeDiffDist.baselineOdds(fn);
		...
		IntStream.range(0, 80)
			.mapToObj(i -> i + " " + fn.value(i) + " " + fn2.value(i))
			.forEach(System.out::println);
	}
