...
public class TestUtils {
	...
	private TestUtils(
		...
		this.outbreak = (ModifiableOutbreak) ExecutionBuilder.buildExperiment(
			ReflectionUtils.merge(
				MINIMAL_SETUP,
				setupTweak.apply(PartialSetupConfiguration.builder())
					.build()
			),
			ReflectionUtils.merge(
				MINIMAL_EXECUTION,
				executionTweak.apply(PartialExecutionConfiguration.builder())
					.build()
			),
			"experiment"
		);
	...
	public static ImmutableErdosReyniConfiguration MINI_COMPLETE = ImmutableErdosReyniConfiguration
		.builder()
		.setNetworkSize(10)
		.setNetworkDegree(9)
		.build();

	public static ImmutableUnstratifiedDemography UNIFORM_RELATIONSHIPS = ImmutableUnstratifiedDemography
		.builder()
		.setRelationshipStrengthDistribution(point(1.0))
		.build();

	/**
	 * A setup for a fully connected network of equal strength relationships that
	 * has 10 nodes and no initial import of disease.
	 */
	public static ImmutableSetupConfiguration MINIMAL_SETUP = ImmutableSetupConfiguration
		.builder()
		.setName("test-setup")
		.setInitialImports(0)
	...
	/**
	 * A minimal in host viral load markov model that has a very short incubation
	 * period, and a exactly 10 day infectious period.
	 */
	public static ImmutableMarkovStateModel MINIMAL_IN_HOST = ImmutableMarkovStateModel
		.builder()
	 ...
	 * parameters fixed to 1 except zero case fatality and case hospitalisation
	 * rate. Default R0 is 4.
	 */
	public static ImmutableExecutionConfiguration MINIMAL_EXECUTION = ImmutableExecutionConfiguration
		.builder()
		.setName("test-execution")
		.setR0(SimpleDistribution.point(4.0))
	...
	public ModifiableOutbreak getOutbreak() { return this.outbreak; }

	public ModifiablePerson getPerson() {
		return (ModifiablePerson) this.getOutbreak()
			.getPeople()
			.get(0);
	}
	...
	public static TestUtils defaultTest() {
		return new TestUtils(setup -> setup, exec -> exec);
	}

	public static ModifiableOutbreak mockOutbreak() {
		return TestUtils.defaultTest()
			.getOutbreak();
	}

	public static ModifiablePerson mockPerson() {
		return (ModifiablePerson) mockOutbreak().getPeople()
			.get(0);
	}
	...
	public static TestUtils defaultWithAdjustments(
		...
		return new TestUtils(setupTweak, executionTweak);
	}

	public static TestUtils defaultWithSetup(
			Function<PartialSetupConfiguration.Builder, PartialSetupConfiguration.Builder> setupTweak
	) {
		return new TestUtils(setupTweak, exec -> exec);
	}
	...
	public static TestUtils defaultWithExecution(
		...
		return new TestUtils(setup -> setup, executionTweak);
	}
	...
	public TestUtils withPatientZero() {
		this.updater = TestUtils.updaterPatientZero();
		return this;
	}
	 ...
	 * 
	 * @return an updater that has a custom rule to expose person 0 with a single
	 *         standard infectious dose at time 1.
	 */
	public static Updater updaterPatientZero() {
		var u = new Updater();
		u.withPersonProcessor(
			pp -> pp.getCurrentState()
				.getTime() == 1 && pp.getId() == 0,
