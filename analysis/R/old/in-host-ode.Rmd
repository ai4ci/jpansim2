...
plot_sweep = function(tmp, facets = ~variable, name) {
  ...
  plt_data = tmp %>% pivot_wider(names_from = variable, values_from=value)
  ...
  ggplot(plt_data, aes(x=virions, y=immune_active, colour = as_factor(set),group=interaction(as.factor(set))))+geom_path()
