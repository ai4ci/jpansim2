...
# modified_p_contact = 
expit(logit(baseline_p_contact)+log(mobilityOR))
```

The problem is that sometimes a low probability of compliance to instruction 
means that the mobility odds (actually the limit of mobility odds) has to be 
modified, to reflect the poor compliance. In this case the mobility odds ratio 
...
$$
logit(P_{eff}) = logit(P_{base}) + log(O_{mob})
$$

The probability of contact will be change by compliance. It will be closer to 
one.

$$
P_{adj} = 1-(1-P_{eff})(1-P_{compliance}) \\
1-P_{adj} = (1-P_{eff})(1-P_{compliance}) \\

logit(P_{adj}) = log(\frac{
  1-(1-P_{eff})(1-P_{compliance}) }{
  (1-P_{eff})(1-P_{compliance})
}) \\
...
$$
1 - P_{eff} = \frac{1-P_{adj}}{1-P_{compliance}}\\
P_{eff} = 1-\frac{1-P_{adj}}{1-P_{compliance}}\\

logit(P_{eff}) = log(\frac{1-\frac{1-P_{adj}}{1-P_{compliance}}}{\frac{1-P_{adj}}{1-P_{compliance}}})\\

logit(P_{eff}) = log(\frac{1-\frac{1-P_{adj}}{1-P_{compliance}}}{\frac{1-P_{adj}}{1-P_{compliance}}})\\

log(\frac{P_{base}}{1-P_{base}})+log(O_{mob})
$$


Mobility is an odds ratio of contact. If the patient is non compliant to 
guidance than the probability of contact will increase compare to if they are
compliant. Odds contact is probability contacted divided by probability not 
contacted. 